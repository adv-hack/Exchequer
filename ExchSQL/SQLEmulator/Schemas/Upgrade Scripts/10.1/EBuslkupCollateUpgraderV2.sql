...
IF EXISTS (
            SELECT *
            FROM   sysobjects tab
            WHERE  (tab.id = object_id('[!ActiveSchema!].EBUSLKUP'))
          )
   AND
   EXISTS (
            SELECT *
...
ALTER TABLE [!ActiveSchema!].[EBUSLKUP] ALTER COLUMN description varchar(100) COLLATE SQL_Latin1_General_CP1_CI_AS
...
SET       Version = 2
WHERE     SchemaName = 'EBUSLKUP_final.xml' AND Version = 1
