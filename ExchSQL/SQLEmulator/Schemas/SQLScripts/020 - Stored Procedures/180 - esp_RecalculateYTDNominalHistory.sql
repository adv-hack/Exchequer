...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[!ActiveSchema!].[esp_RecalculateYTDNominalHistory]') AND type in (N'P', N'PC'))
DROP PROCEDURE [!ActiveSchema!].[esp_RecalculateYTDNominalHistory]
GO

-- This currently checks YTD for CC/Dept Only - need to look at all YTD inc. ProfitBF Control

CREATE PROCEDURE !ActiveSchema!.esp_RecalculateYTDNominalHistory
AS
BEGIN
