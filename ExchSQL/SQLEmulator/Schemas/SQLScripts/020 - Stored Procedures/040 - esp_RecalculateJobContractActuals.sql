/****** Object:  StoredProcedure [!ActiveSchema!].[esp_RecalculateJobContractActuals]    Script Date: 02/04/2015 12:21:16 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[!ActiveSchema!].[esp_RecalculateJobContractActuals]') AND type in (N'P', N'PC'))
DROP PROCEDURE [!ActiveSchema!].[esp_RecalculateJobContractActuals]
GO
...
-- Recalculates Job and Contract History Actuals 
...
-- Usage: EXEC !ActiveSchema!.esp_RecalculateJobContractActuals 1

CREATE PROCEDURE [!ActiveSchema!].[esp_RecalculateJobContractActuals] ( @iv_Mode    INT
...
DECLARE @c_Fix            INT = 0
      ...
      , @v_QueryStartTime DATETIME = GETDATE()
...
AND   JA.Cost <> 0
AND   JA.AnalysisCode <> ''
AND   TT.TransactionTypeCode NOT IN ('SOR','POR','SDN','PDN', 'JCT', 'JST') /* Rule 3 */
...
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Initial Data Load - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  ...
  SET @v_QueryStartTime = GETDATE()
END

-- Add Job Ascendants
INSERT INTO #RecalcJobActual
SELECT RJA.HistArray
...
FROM   #RecalcJobActual RJA (NOLOCK)
JOIN   !ActiveSchema!.evw_JobAscendant JASC ON JASC.JobCode = RJA.JobCode COLLATE SQL_Latin1_General_CP1_CI_AS
WHERE  JASC.AscendantJobCode <> JASC.JobCode

IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Parent Data Load - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Set Analysis Ids - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Insert Parent Analysis - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Switch Sales & Purchase for Revenue - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Insert WIP (170) - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Insert Profit (99) - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: Summarise Job History - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
BEGIN
  SELECT JH.HistoryPositionId
       , NJH.JobCode
       , HistoryClassificationId = ASCII(JOB.JobContractTypeCode)
  ...
  LEFT JOIN !ActiveSchema!.evw_JobHistory JH ON NJH.JobCode            = JH.JobCode            COLLATE SQL_Latin1_General_CP1_CI_AS
  ...
  SET @v_InfoMessage  = 'JobContractActuals: Report Data - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
BEGIN
  ...
  --SET @v_InfoMessage  = 'JobContractActuals: Fix Data - Query duration - %s. Duration to this point - %s.'
  --SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  --SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
...
IF @iv_Mode IN (@c_Report, @c_Debug)
BEGIN
  SET @v_InfoMessage  = 'JobContractActuals: End - Query duration - %s. Duration to this point - %s.'
  SET @v_QueryMessage = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @v_QueryStartTime))
  SET @v_ProcMessage  = CONVERT(VARCHAR(20), CONVERT(TIME, GETDATE() - @c_ProcStartTime))
