/****** Object:  StoredProcedure [!ActiveSchema!].[esp_RecalculateCTDJobActuals]    Script Date: 21/01/2015 08:46:30 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[!ActiveSchema!].[esp_RecalculateCTDJobActuals]') AND type in (N'P', N'PC'))
DROP PROCEDURE [!ActiveSchema!].[esp_RecalculateCTDJobActuals]
GO

-- Recalculates the Current to Date Value (255) for Job Actuals

-- USAGE: EXEC !ActiveSchema!.esp_RecalculateCTDJobActuals

CREATE PROC !ActiveSchema!.esp_RecalculateCTDJobActuals
AS
BEGIN
