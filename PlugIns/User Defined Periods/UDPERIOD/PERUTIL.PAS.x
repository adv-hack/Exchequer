...
type
  TPeriodCalcError = (perOK, perNoData, perDateOutOfRange, perErrorInData);
...
uses
  Forms, SysUtils, IniFiles, Dialogs, Windows;
...
//  with TIniFile.Create(ExtractFilePath(Application.ExeName) + PERIODS_INI_FILE) do
    try
      ...
      UsePlugIn := UpperCase(ReadString('Settings', 'UsePlugIn', 'TRUE')) = 'TRUE';
    finally
      Free;
    end;
