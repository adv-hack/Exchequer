...
//PR: 21/08/2012 ABSEXCH-12843 All access to MiscF has been changed to work without client ids. See comment in AdmnUtil.pas
...
Begin
  ...
  Status:=Find_RecCID(B_GetEq,F[Fnum],Fnum,RecPtr[Fnum]^,KeyPath,KeyS, nil);
...
Begin
  ...
  begin
    GetPosCId(F[FNum],Fnum,LAddr,nil);
    Move(LAddr, RecPtr[FNum]^, SizeOf(LAddr));
//    Ok:=GetMultiRecAddr(B_GetDirect,B_MultLock,KeyS,Keypath,Fnum,BOn,Locked,LAddr);
    Res := GetDirectCId(F[FNum], FNum, RecPtr[FNum]^, KeyPath, B_MultNWLock, nil);
    OK := Res = 0;
    Locked := OK;
  end;
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        If (NewRec) then
          Status:=Add_RecCID(F[Fnum],Fnum,RecPtr[Fnum]^,KeyPath, nil)
        else
        Begin
          Status:=Put_RecCID(F[Fnum],Fnum,RecPtr[Fnum]^,KeyPath, nil);

          If (StatusOk) then
            Status:=UnLockMultiSingCID(F[Fnum],Fnum,LAddr, nil);
        end;
      end
      else
        Status:=Delete_RecCID(F[Fnum],Fnum,KeyPath, nil);
    end
    else
      Status:=UnLockMultiSingCID(F[Fnum],Fnum,LAddr, nil);
...
Begin
  With Source do
  If (Pred(GetMenuItemCount(Handle))>=0) or (Count>0) then
  Begin
...
Begin
  With Source do
  Begin
    n:=Pred(GetMenuItemCount(Handle));
    While n>=0 do
    Begin
      Items[0].Free;
      n:=Pred(GetMenuItemCount(Handle));
...
Begin
  With Source do
    CreateIdxSubMenu(Source,Pred(GetMenuItemCount(Handle)),Dest,Suffix,MatchVis);
end;
...
Begin
  If (RefCount<>-1) then
    ...
    Loop:=GetMenuItemCount(Source.Handle)-1;
