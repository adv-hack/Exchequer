...
Type
  TEbusDetlDataWrite = Class(TBaseDataWrite)
  Private
    FADOQuery : TADOQuery;
...
Constructor TEbusDetlDataWrite.Create;
Begin // Create
  Inherited Create;
...
Destructor TEbusDetlDataWrite.Destroy;
Begin // Destroy
...
// Called from the SQL Write Threads to provide basic info required for SQL Execution
Procedure TEbusDetlDataWrite.Prepare (Const ADOConnection : TADOConnection; Const CompanyCode : ShortString);
Var
  sqlQuery : ANSIString;
Begin // Prepare
  ...
  sqlQuery := 'INSERT INTO [COMPANY].EBUSDETL (' + 
                                              'tlFolioNum, ' + 
                                              'tlLineNo, ' + 
                                              'tlRunNo, ' + 
                                              'tlGLCode, ' + 
...
Procedure TEbusDetlDataWrite.SetQueryValues (Var ADOQuery : TADOQuery; Const DataPacket : TDataPacket; Var SkipRecord : Boolean);
Var
  DataRec : ^Idetail;
