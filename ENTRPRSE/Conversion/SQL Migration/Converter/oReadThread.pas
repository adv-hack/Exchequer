...
type
  ...
  protected
    function CheckPreReqs(Const oCompany : TConversionCompany; Const oTask : IDataConversionTask) : Boolean;
...
Uses oGenericBtrieveFile, oDataPacket, oWriteThread, LoggingUtils, oBaseDataWrite;
...
//PR: 07/09/2016 ABSEXCH-15014 Check if we need to create a table at this point. At this time only HistPrge needs to be created
//if it exists in any of the Pervasive companies. Other tables can be catered for by adding a new descendant of TCreateTable in
//oBaseDataWrite.pas and extending the case statement here.
function TBtrieveReadThread.CheckPreReqs(Const oCompany : TConversionCompany; Const oTask : IDataConversionTask) : Boolean;
var
  CreateTable : TCreateTable;
begin
  Result := True;
  Case oTask.dctTaskID of
    dmtHistPurge : CreateTable := TCreatePurgeHistory.Create;
                   else
                     CreateTable := nil;
  end; //Case
  if Assigned(CreateTable) then
    Try
      Try
        CreateTable.Execute(oCompany);
      Finally
        FreeAndNil(CreateTable);
      End;
    Except
      Result := False;
      raise; //Pass exception details back to calling procedure for logging
    End;
end;
