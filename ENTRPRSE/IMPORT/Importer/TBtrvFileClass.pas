...
{* getters and setters *}
  ...
  public
    ...
    function  FindFirst(var InputRec): integer; overload;
    function  FindFirst(var InputRec; Key: Str255): integer; overload;
    function  FindGEq(var InputRec; Key: Str255): integer; 
...
uses sysutils, dialogs{, TErrors, TLoggerClass}, Forms, SQLUtils;
...
begin
  ...
  FFileIx       := MAXFILES;       // default, from Btrvu2
end;
...
// n.b. RecPtr[FileIx] is set in WriteRecord as it can be different with each call
...
begin
  ...
  if not FBtrvFileOpen then
    result := OpenFile; // first call to OpenFile defines the file with DefineBtrvFile
...
function TBtrvFile.FindFirst(var InputRec; Key: Str255): integer;
begin
  Result := 0;
  if not FBtrvFileOpen then
    result := OpenFile; // first call to OpenFile defines the file with DefineBtrvFile
  ...
  FKeyS := '';
  RecPtr[FileIx] := @InputRec;
  Result := Find_Rec(B_GetFirst, FBtrvFileF, FileIx, RecPtr[FileIx]^, 0 , Key);
end;

//HV 10/07/2018 2018R1.1 ABSEXCH-20940: changes made to address performance issue for Windows Authentication
function  TBtrvFile.FindGEq(var InputRec; Key: Str255): integer;
begin
  Result := 0;
  if not FBtrvFileOpen then
    Result := OpenFile; // first call to OpenFile defines the file with DefineBtrvFile
  ...
  FKeyS := '';
  RecPtr[FileIx] := @InputRec;
  result := Find_Rec(B_GetGEq, FBtrvFileF, FileIx, RecPtr[FileIx]^, 0 , Key);
end;
...
begin
  ...
  FKeyS := Key;
  RecPtr[FileIx] := @InputRec;
  result := Find_Rec(B_GetEq, FBtrvFileF, FileIx, RecPtr[FileIx]^, 0 , FKeyS);
end;
...
begin
  ...
  FKeyS := Key;
  RecPtr[FileIx] := @InputRec;
  result := Find_Rec(B_GetEq, FBtrvFileF, FileIx, RecPtr[FileIx]^, KeyNo , FKeyS);
end;
...
begin
  ...
  if not FBtrvFileDefined then
    if DefineBtrvFile <> 0 then exit; // first call to OpenFile defines the file

  //PR: 07/08/2013 ABSEXCH-14500
  //Change so that we no longer use TableExists to check for the existence of the file -
  //caching could lead to FileExists returning false, even though the Make_File had returned 0.
  ...
  begin
    ...
    begin
      ...
      if rc <> 0 then
        FLastError := format('Open_File failed: %d', [rc]);
    end
    else
      FLastError := format('Make_File failed: %d', [rc])
  end
  else
    if rc <> 0 then
       FLastError := format('Open_File failed: %d', [rc]);
...
// callers don't need to explicitly call DefineBtrvFile and OpenFile when writing
...
begin
  ...
  if result <> 0 then FLastError := format('Add_Rec failed: %d', [result]);
end;
