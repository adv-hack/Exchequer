...
Function PrintToFile (P, P2       : Pointer;
                      PSize       : LongInt;
                      AttType     : Byte;
                      AttachmentPrinter : string = '') : SmallInt;  //PR: 16/02/2011 ABSEXCH-10913


implementation

uses
  SysUtils, ElVar, PrintFrm, VarConst;
...
//PR: 04/04/2013 ABSEXCH-12692 Added function to find correct print mode according to doc type.
function GetPrintMode(const OurRef : string) : Integer;
var
  dt : DocTypes;
  s : string;
begin
  //Default result to 1, which covers most transaction types
  Result := 1;

  //Find DocType from first 3 chars of Our Ref
  s := Copy(OurRef, 1, 3);
  for dt := Low(DocTypes) to High(DocTypes) do
  if s = DocCodes[dt] then
  begin
    //Found - return print mode
    Result := DEFDEFMode[dt];
    Break;
  end;
end;
...
begin
  with PBatch do
  begin
    //PR: 04/04/2013 ABSEXCH-12692
    pbDefMode   := GetPrintMode(OurRef);              { Mode of form }
...
Function PrintToFile (P, P2       : Pointer;
                      PSize       : LongInt;
                      AttType     : Byte;
                      AttachmentPrinter : string = '') : SmallInt;
Var
        ...
        // Check SBSFORM.DLL is loaded and happy
          ...
          { RPDev.DeviceIndex := RPDev.DeviceIndex;}
            ...
            Begin //PR: 16/02/2011 ABSEXCH-10913
              if (AttType = 1) and (Trim(AttachmentPrinter) <> '') then //Adobe PDF
                DevIdx := pfFind_DefaultPrinter(AttachmentPrinter)
              else
