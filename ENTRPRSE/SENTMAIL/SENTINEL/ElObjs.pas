...
uses
  Classes, ElVar, btbase, Btrvu2, ETDateU, SysUtils, dialogs, EngineF;
...
{  private
  ...
  protected
    ...
    FCompanyCode : String;
    FKeyS : string[255];  //PR: 11/11/2010
  ...
  private
    ...
    FType : Char;
    FPriority : Char;
    //PR: 05/04/2011 ABSEXCH-2878 When any date or time related fields are changed, set this so that a new next run time
    //is calculated
    FDateTimeChanged : Boolean;
  protected
  ...
  public
    ...
    property DateTimeChanged : Boolean index 30 read GetBoolean write SetBoolean;
  end;
...
begin
  ...
  //PR: 11/11/2010 Change to pass in returned KeyS - caused a problem in sql if we didn't
  if SearchType in [B_GetNext, B_GetPrev] then
    KeyS := FKeyS
  else
  ...
  //PR: 11/11/2010 Store returned KeyS
  if Result = 0 then
    FKeyS := KeyS;
...
begin     //PR: 05/04/2011 ABSEXCH-2878
  if FDataPointer^.elDaysOfWeek <> Value then
  begin
    FDataPointer^.elDaysOfWeek := Value;
    FDateTimeChanged := True;
  end;
end;
...
begin       //PR: 05/04/2011 ABSEXCH-2878
  if FDataPointer^.elTimeType <> Ord(Value) then
  begin
...
begin
  Case Index of
    ...
    9  : if FDataPointer^.elPeriodic <> Value then
         begin      //PR: 05/04/2011 ABSEXCH-2878
           FDataPointer^.elPeriodic := Value;
           FDateTimeChanged := True;
         end;
...
begin
  Case Index of
    1  : if FDataPointer^.elTime1 <> Value then
         begin      //PR: 05/04/2011 ABSEXCH-2878
           FDataPointer^.elTime1 := Value;
           FDateTimeChanged := True;
         end;
    2  : if FDataPointer^.elTime2 <> Value then
         begin      //PR: 05/04/2011 ABSEXCH-2878
           FDataPointer^.elTime2 := Value;
           FDateTimeChanged := True;
         end;
...
begin
  ...
  KeyS := BuildSearchKey(SearchKey);
  FillChar(FDataBuffer, SizeOf(FDataBuffer), #0);
  Result := Find_Rec(SearchType, FVar,FFileNo,RecPtr[FFileNo]^, FIndex, KeyS);
  if Result = 0 then
    MoveDataForRead;
end;
...
function TSysRec.BuildSearchKey(const Key: string): ShortString;
var
  s : ShortString;
  i : SmallInt;
begin
  ...
  Move(i, s[1], SizeOf(i));
  Result := FPrefix + {LJVar(FUserID, 10)  + LJVar(FElertName, 30)}
             MakeElertNameKey(FUserID, FElertName) + #0#0 + FDataPointer^.eoOutputType + #0#0#0#0'!';
end;
