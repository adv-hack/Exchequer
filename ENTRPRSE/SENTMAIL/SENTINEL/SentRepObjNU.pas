...
DBug.Msg (0, 'End RepNomCtrlObj.Process_Report');}
  ...
  Constructor RepRunCtrlObj.Create(AOwner : TObject;RepCRec : RepGenRec; xInfo : TExtraRepInfoRec);
  {Constructor RepRunCtrlObj.Init(RepCRec  :  ReportHedType);}

  Begin
    Inherited Create (Aowner, RepCRec, xInfo);

    {RepNomCtrlObj.Init(RepCRec);}

//    NomMode:=(RepCtrlRec^.RepType=RepNomCode);
//moved to Process_file
...
procedure RepRunCtrlObj.InitInThread;
begin
  inherited InitInThread;
  NomMode:=(RepCtrlRec^.RepType=RepNomCode);
end;
