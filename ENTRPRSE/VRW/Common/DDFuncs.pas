...
function UseDDFieldCache: Boolean;
begin
  Result := True;
end;
...
function GetDDFieldForCache(const ShortCode : ShortString;
                            var   DataRec   : DataDictRec) : Boolean;
const
  FNum    = DictF;
  KeyPath = DIK;
...
begin
  ...
  if UseDDFieldCache then
  begin
    if (DDFieldList = nil) then
    begin
      DDFieldList := TStringList.Create;
      DDFieldList.Sorted := True;
    end;

    FieldPos := DDFieldList.IndexOf(Keys);
    if (FieldPos = -1) then
    begin
      if GetDDFieldForCache(ShortCode, DataRec) then
      begin
        DictObj := TDDFieldObj.Create;
        DictObj.DataRec := DictRec^;
        DDFieldList.AddObject(Keys, DictObj);
        Result := True;
      end
      else
        Result := False;
    ...
    begin
      DataRec := TDDFieldObj(DDFieldList.Objects[FieldPos]).DataRec;
      Result := True;
    end;
  end
  else
    Result := GetDDFieldForCache(ShortCode, DataRec);
end;
