...
Type
  TRegionManager = Class(TInterfacedObject, IRegionManager)
  Private
    ...
    // FPaperSizeHeight is the height of the paper, e.g. 297mm for A4
    FPaperSizeHeight : SmallInt;
    // FPaperSizeWidth is the width of the paper, e.g. 210mm for A4
    FPaperSizeWidth : SmallInt;
    // FPaperWidthInMM is the current width of paper in use, this will be either
    // FPaperSizeHeight or FPaperSizeWidth depending on the reports Paper Orientation
    FPaperWidthInMM : SmallInt;
...
Begin // Create
  ...
  Begin
    Try
      // Colours
      FBannerColor := ReadInteger(EntryRec^.Login, 'BannerColour', $00F2E6EE); // B:146 G:98 R:106
      FBannerFont.Color := ReadInteger(EntryRec^.Login, 'BannerFontColour', $0092626A); // B:242 G:230 R:238
...
begin
  ...
  Begin
    ...
    Begin
      ...
      // Sorting
      If (cciSortingSubMenu In MenuItemSet) And (TControl(FSelectedControls[I]) Is TVRWDBFieldControl) Then
      Begin
        With TVRWDBFieldControl(FSelectedControls[I]).DBFieldDets Do
        Begin
...
Begin // DisplayControlOptions
  ...
  Begin
    DisplayTextOptions (TVRWTextControl(TheControl));
...
Begin // RealignRegions
  ...
  Begin
    ...
    Begin
      ...
      RegionCtrl.Width := FPaperWidthInMM * PixelsPerMM + LeftBorderWidth + RightBorderWidth;
...
// Called from the Region and Controls to get the Controls Tree window displayed
Procedure TRegionManager.ShowControlsTree;
Var
...
Begin // SetPaperOrientation
  ...
  If (Value = poLandscape) Then
    SetPaperWidth(FPaperSizeHeight)
  Else
    SetPaperWidth(FPaperSizeWidth);
...
Begin // GetPaperSizeHeight
  Result := FPaperSizeHeight;
End; // GetPaperSizeHeight
Procedure TRegionManager.SetPaperSizeHeight (Value : SmallInt);
Begin // SetPaperSizeHeight
  FPaperSizeHeight := Value;
  If (GetPaperOrientation = poLandscape) Then
  Begin
    SetPaperWidth(FPaperSizeHeight);
  End; // If (FPaperOrientation = poLandscape)
End; // SetPaperSizeHeight

//------------------------------

Function TRegionManager.GetPaperSizeWidth : SmallInt;
Begin // GetPaperSizeWidth
  Result := FPaperSizeWidth;
End; // GetPaperSizeWidth
Procedure TRegionManager.SetPaperSizeWidth (Value : SmallInt);
Begin // SetPaperSizeWidth
  FPaperSizeWidth := Value;
  If (GetPaperOrientation = poPortrait) Then
  Begin
    SetPaperWidth(FPaperSizeWidth);
  End; // If (FPaperOrientation = poLandscape)
End; // SetPaperSizeWidth

//------------------------------

Function TRegionManager.GetPaperWidth : SmallInt;
Begin // GetPaperWidth
  Result := FPaperWidthInMM;
End; // GetPaperWidth
Procedure TRegionManager.SetPaperWidth (Value : SmallInt);
Begin // SetPaperWidth
  If (FPaperWidthInMM <> Value) Then
  Begin
    FPaperWidthInMM := Value;

    // Update Regions
    RealignRegions;
  End; // If (FPaperWidthInMM <> Value)
End; // SetPaperWidth
...
// Prints the current report
Procedure TRegionManager.PrintReport;
Begin // PrintReport
...
// NOTE: This should be rewritten to use a while loop as the controls
// need to be removed as they are copied into the clipboard - see
// DeleteControls below

  If (FSelectedControls.Count > 0) Then
  Begin
    For I := 0 To (FSelectedControls.Count - 1) Do
    Begin
      //TWinControl(FSelectedControls.Items[I]).?
      ShowMessage ('Cut ' + TWinControl(FSelectedControls.Items[I]).Name);
    End; // For I
  End; // If (FSelectedControls.Count > 0)
End; // CutControls
...
procedure TRegionManager.AlignControls(Horizontal,
  ...
  begin
    if (GetPaperOrientation = poLandscape) then
      MinX := FPaperSizeHeight * PixelsPerMM
    else
      MinX := FPaperSizeWidth * PixelsPerMM;
  ...
  begin
    if (GetPaperOrientation = poLandscape) then
      MinY := FPaperSizeWidth * PixelsPerMM
    else
      MinY := FPaperSizeHeight * PixelsPerMM;
    for i := 0 to (Controls.Count - 1) do
    begin
    ...
    try
      ...
      case Horizontal of
        ...
        caHorzRegionCentre:
          begin
            if (GetPaperOrientation = poLandscape) then
              X := FPaperSizeHeight div 2
            else
              X := FPaperSizeWidth div 2;
            X := X * PixelsPerMM;
          end;
      end;
      case Vertical of
