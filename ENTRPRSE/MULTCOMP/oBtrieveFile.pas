...
Type
  ...
  //PR: 17/10/2017 ABSEXCH-19303 Return record for STAT EXTENDED call,
  ...
  Private
    ...
    // MH 07/04/2016 2016-R2: Added Task ID so it can be used to link Btrieve File objects into the same clientID
    Function GetClientTaskID : Integer;

    // MH 06/04/2016 2016-R2: Added support for Database Transactions
    Function DBTransactionOp (Const OpCode : Integer) : Integer;
  Protected
    FDataRec     : Pointer;
    FDataRecLen  : LongInt;
    FDataFile    : Pointer;
    ...
    // Overridden in descendants to automatically pad indexed fields to their correct
    // length when Insert or Update are called
    Procedure PadIndexFields; Virtual;
  Public
    ...
    // MH 07/04/2016 2016-R2: Added Task ID so it can be used to link Btrieve File objects into the same clientID
    Property ClientTaskID : Integer Read GetClientTaskID;
    //PR: 10/10/2016 2017-R1 Added Index property
    ...
    Function OpenFile (Const OpenFilePath : ShortString = ''; Const CreateIfMissing : Boolean = False; Const OwnerName : ShortString = v600Owner; Const Mode : SmallInt = 0) : LongInt;

    // Public: Closes the data file, returns 0 if successful.  Assumes that the Client-Id is specific
    // to a single file and will automatically reset the Client-Id in the SQL Emulator to avoid
    // caching problems unless ResetCId is passed in as False.
    Function CloseFile (Const ResetCId : Boolean = True) : LongInt;
    ...
    function KeyExists(const SearchKey: WideString): Integer;
    ...
    // MH 06/04/2016 2016-R2: Added support for Database Transactions
    Function StartDBTransaction : Integer;
    Function CommitDBTransaction : Integer;
    Function AbortDBTransaction : Integer;

    //PR: 17/10/2017 ABSEXCH-19303 STAT EXTENDED call, subfunction 4 - File Information
    Function StatExtendedFileInfo(var FileInfo : TStatExtFileInfo) : Integer;
...
//  1-9999   Btrieve Error opening data file
//  10001    Btrieve Not Available
//  11000+   Error in Open Company
Function TBaseBtrieveFile.OpenFile (Const OpenFilePath : ShortString = ''; Const CreateIfMissing : Boolean = False; Const OwnerName : ShortString = v600Owner; Const Mode : SmallInt = 0) : LongInt;
Var
  lStat : Integer;
  SetOwnerAfterOpen : Boolean;
Begin // OpenFile
  If Check4BtrvOK Then
  Begin
    SetOwnerAfterOpen := False;

    If (OpenFilePath <> '') Then
      SetFilePath (OpenFilePath);

    // MH 05/11/07: Directory won't necessarily exist under SQL
    //If DirectoryExists (ExtractFilePath(FFilePath)) Then
    If UsingSQL Or DirectoryExists (ExtractFilePath(FFilePath)) Then
    Begin
      // MH 18/09/2008: Added an option to skip OpenCompany as it was necessary in the MSSQL to P.SQL
      // converter in order to be able to open Company.Dat and correct the paths.
      If (Not FBypassOpenCompany) Then
        // MH 05/11/07: Need to tell the emulator the path before we can check for tables existing
        lStat := OpenCompany(ExtractFilePath(FFilePath), FClientID)
      Else
        lStat := 0;
      ...
      Begin
        ...
        If CreateIfMissing And (Not TableExists (FFilePath)) Then
        Begin
          // Create the data file
          Result := Make_FileCId(FFileVar, FFilePath, FDataFile^, FDataFileLen, FClientId);

          // MH 07/04/2014 v7.X Auto-Receipt: Added automatic setting of owner name when a file is created
          SetOwnerAfterOpen := (Result = 0) And (OwnerName <> '');
        End; // If CreateIfMissing And (Not TableExists (FFilePath))

        If (Result = 0) Then
        Begin
          If TableExists (FFilePath) Then
          Begin
            // Open the file
            ...
            Begin
              ...
              // MH 07/04/2014 v7.X Auto-Receipt: Added automatic setting of owner name when a file is created
              If SetOwnerAfterOpen Then
              Begin
                Result := SetOwner (OwnerName, 3);   // Allow read-only access without owner name
              End; // If SetOwnerAfterOpen
            End; // If (Result = 0)
          End // If TableExists (FFilePath)
          Else
...
Function TBaseBtrieveFile.CloseFile (Const ResetCId : Boolean = True) : LongInt;
Var
...
Begin // CloseFile
  ...
  If SQLUtils.UsingSQL And ResetCId Then
  Begin
    // Reset the Client-Id session to avoid SQL Emulator caching problems
    CloseClientIdSession(FClientId);
...
// Overridden in descendants to automatically pad indexed fields to their correct
// length when Insert or Update are called
Procedure TBaseBtrieveFile.PadIndexFields;
Begin // PadIndexFields
...
Function TBaseBtrieveFile.Update : LongInt;
Var
  CurIndKey  : Str255;
  Tries      : SmallInt;
Begin // Update
  ...
  // Positioned on locked record - try to update it
  Tries := 0;
  Repeat
    Result := Btrv (B_Update, FFileVar, FDataRec^, FDataRecLen, CurIndKey[1], FIndex, FClientId);
    Inc(Tries);
  Until (Not (Result In [84,85])) or (Tries>TryMax);
End; // Update
...
Begin // FindRec
  ...
  // MH 05/11/07: Modified to store record length in local variable as the emulator keeps changing it
  C := FDataRecLen;
  //Result := Btrv(BFunc, FFileVar, FDataRec^, FDataRecLen, SearchKey[1], FIndex, FClientId);
  Result := Btrv(BFunc, FFileVar, FDataRec^, C, SearchKey[1], FIndex, FClientId);
...
function TBaseBtrieveFile.KeyExists(const SearchKey: WideString): Integer;
begin
  Result := GetDataRecord (B_GetEq + B_KeyOnly, SearchKey);
end;
...
Var
  ...
  RecCnt   : LongInt;
  I, Index, IndexSegment, FileBVer : Byte;
...
Begin // LoadKeySegs
  ...
  Begin
    ...
    CheckForOverlaps;
    Result := FKeySegments.Count;
  End // If (File_StatCId(FFileVar, FClientId, RecCnt, FileBVer, @KeySpecs) = 0)
  Else
    Result := 0;
End; // LoadKeySegs
...
// MH 07/04/2016 2016-R2: Added Task ID so it can be used to link Btrieve File objects into the same clientID
Function TBaseBtrieveFile.GetClientTaskID : Integer;
Begin // GetClientTaskID
  Result := FClientId.TaskId;
End; // GetClientTaskID

//-------------------------------------------------------------------------

Function TBaseBtrieveFile.DBTransactionOp (Const OpCode : Integer) : Integer;
Var
  DumKeyNum,DumRecLen  :  Integer;
  DumDataRec           :  array[1..2] of Char;
  FileB                :  FileVar;
  DumKey               :  String[1];
Begin
  // Setup dummy parameters for everything other than the OpCode and ClientID
  FillChar(DumDataRec,Sizeof(DumDataRec),0);
  FillChar(DumKey,Sizeof(DumKey),0);
  ...
  Result:=Btrv(OpCode, FileB, DumDataRec, DumRecLen, DumKey[1], DumKeyNum, FClientId);
end;

//-----------------------------------

// MH 06/04/2016 2016-R2: Added support for Database Transactions
Function TBaseBtrieveFile.StartDBTransaction : Integer;
Begin // StartDBTransaction
  Result := DBTransactionOp (B_BeginTrans);
End; // StartDBTransaction

//-----------------------------------

Function TBaseBtrieveFile.CommitDBTransaction : Integer;
Begin // CommitDBTransaction
  Result := DBTransactionOp (B_EndTrans);
End; // CommitDBTransaction

//-----------------------------------

Function TBaseBtrieveFile.AbortDBTransaction : Integer;
Begin // AbortDBTransaction
  Result := DBTransactionOp (B_AbortTrans);
End; // AbortDBTransaction
...
//=========================================================================
//PR: 17/10/2017 ABSEXCH-19303 STAT EXTENDED call, subfunction 4 - File Information
function TBaseBtrieveFile.StatExtendedFileInfo(
  var FileInfo: TStatExtFileInfo): Integer;
var
  DumKey : Str255;
  DumKeyNum : longint;
  DumLen : longint;
begin
  ...
  Result := Btrv(65, FFileVar, FileInfo, DumLen, DumKey, DumKeyNum, FClientID);
end;
//=========================================================================
