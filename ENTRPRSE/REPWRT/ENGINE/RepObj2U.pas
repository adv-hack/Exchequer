...
Type
  ...
  RepFieldRObj  =  Object
                     ...
                     Function GetSortStr  :  ShortString;
...
{        else
    ...
    {PR 22/03/05 Change so that -ve values are sorted correctly (i.e. before zero!) -
     do this by setting the first char of the string to be char before '0' when required}
    if ((DValue < 0) and not Decend) or
       ((DValue > 0) and Decend) then
      SortNumber[1] := Pred('0');
  ...
  Begin
    ...
    //PR: 7/3/05 - changed to make sure equals/not equals works correctly
{    TmpBo:=(((Cond1<Cond2) and (Operand=1)) or
    ...
    Case Operand of
      ...
      3  : TmpBo := Cond1<=Cond2;
      4  : TmpBo := Cond1>=Cond2;
      5  : TmpBo := UpperCase(Cond1) <> UpperCase(Cond2);
      6  : TmpBo := UpperCase(Cond1) = UpperCase(Cond2);
      //Begins with
      7  : TmpBo := Pos(Cond2, Cond1) = 1;
      //Contains
      8: TmpBo := Pos(Cond2, Cond1) > 0;
      //Ends with
      //PR: 18/03/2009 EndsWith wasn't working correctly if there was more than one occurrence of the substring
      9: TmpBo := Copy(Cond1, Length(Cond1) - (Length(Cond2) - 1), Length(Cond1)) = Cond2;
    end; //case
...
begin
  ...
  Function RepFieldRObj.GetSortStr  :  ShortString;

  Var
    Dnum  :  Double;
    TStr  :  AnsiString;
    Buf   : Array[0..161] of Char;
    //PR: 24/05/2010 Straightforward stringlist sort wouldn't deal with non-printable chars, so convert to hex strings.
    function StrToAscii : AnsiString;
    begin
      //PR: 13/06/2017 ABSEXCH-15426 v2017 R2 Need to blank out Buf to avoid rubbish chars spoiling sort
      FillChar(Buf, SizeOf(Buf), 0);
      BinToHex(PChar(TStr), Buf, Length(TSTr));
  ...
  Begin
    ...
    Begin
      ...
      begin
        if Pos('"', RepDet.VarSubSplit) <> 1 then
          RepDict.VarType := 2;
      end;
