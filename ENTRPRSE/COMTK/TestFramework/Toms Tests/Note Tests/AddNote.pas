...
type TAddDepartmentNote = class(TAddNote)
public
 function FindParentInterface : integer; override;
end;

type TAddSupplierNote = class(TAddNote)
public
 function FindParentInterface : integer; override;
end;
...
begin
{
 ...
 Transaction (SQU/PQU/SOR/POR/SIN/PIN/NOM/ADJ/TSH)
  ...
  Case docType of
    ...
    9 : Result := TAddDepartmentNote.Create;
    10 : Result := TAddSupplierNote.Create;
    else
      Result := nil;
  end;
end;
...
function TAddDepartmentNote.FindParentInterface;
var
 searchKey : string;
begin
  with fToolkit.Department as ICCDept3 do
  begin
   Index := acIdxCode;
   searchKey := BuildCodeIndex('BRO');
   result := GetEqual(searchKey);
...
function TAddSupplierNote.FindParentInterface;
var
 searchKey : string;
begin
