...
Begin VB.Form Form1 
   ...
   Caption         =   "COM TK Open/Close Sledge Hammer"
   ClientHeight    =   1800
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5160
...
Dim sTime$
sTime$ = Time$
MsgBox sTime$ + "-" + Mid$(sTime, Len(sTime), 1)
End Sub
...
Private Sub Timer1_Timer()
    ...
    Do
        sTime$ = Time$
        If (Mid$(sTime, Len(sTime), 1) = "0") Then
            With oToolkit
        ...
        End If ' (Mid$(sTime, Len(sTime), 1) = "0")
        
        ' Do some background processing to pick up close messages, etc...
        DoEvents
    Loop Until (chkRunning.Value = 0)
