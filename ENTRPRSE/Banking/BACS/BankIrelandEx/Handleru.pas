...
Const
  ...
  DefaultPayFileName = 'BI_Pay.dat';
  DefaultRecFileName = 'BI_DDeb.dat';
...
Begin
  ...
  if HookIsLicenced(exBnkIreEx) then
  { Enable Hooks and Set About Message here }
  with CustomHandlers do
  begin
    MakeBacsAboutText(exBnkIreEx);
...
Begin
  ...
  if (WinID = BacsHook) then
    HandleBatchHooks(EventData, HandlerID, exBnkIreEx, ftaBoth, DefaultPayFilename,
                     DefaultRecFileName);
