...
Uses Dialogs, SysUtils, ExWrap, Forms, Classes, BacConst;
...
Const
  ...
  DefaultPayFileName = 'Credit.csv';
  DefaultRecFileName = 'Debit.csv';
...
Begin
  ...
  if HookIsLicenced(exFirstTrust) then
  { Enable Hooks and Set About Message here }
  with CustomHandlers do
  begin
    MakeBacsAboutText(exFirstTrust);
...
Begin
  ...
  if (WinID = BacsHook) then
    HandleBatchHooks(EventData, HandlerID, exFirstTrust, ftaBoth, DefaultPayFilename,
                     DefaultRecFileName);
