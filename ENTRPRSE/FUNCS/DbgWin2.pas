...
function DBug: TDebugObject;

implementation

uses SysUtils, ActiveX, Windows, ComObj, Dialogs;
...
function DBug: TDebugObject;
begin
  if not Assigned(FDbug) then
    FDbug := TDebugObject.Create;
  Result := FDbug;
end;
...
//    Svr := CreateOLEObject('Sysmessg.DebugServer') as IDebugServer;
    Svr.Clear;
    Svr.AddUsage;
    Svr.DisplayMsg(1, 'Connected to Debug Server')
  except
    Svr := nil;
  end;
  {$ENDIF}
