...
begin
  ...
  {$IF Defined(EX600) or Defined(LIC600) or Defined(EXSQL)}
    ...
    or (UpperCase(Copy(sVersionNo,1,2)) = 'B6')
    // MH 04/01/2013: Extended for v7.0 for NF
    or (UpperCase(Copy(sVersionNo,1,2)) = 'V7')
    or (UpperCase(Copy(sVersionNo,1,2)) = 'B7')
    // MH 30/07/2015 2015-R1: Extended for Exchequer 2015 R1 type versioning
    Or (UpperCase(Copy(sVersionNo,1,12)) = 'EXCHEQUER 20') Then
    begin
      // this is OK. It is a v6/v7 plug-in, and it has v6 compiler directives defined
    end else
    begin
      ...
      begin
        ...
        + 'You are compiling a v5 plug-in, but you have v6/v7+ compiler directives defined'
        , mtError, [mbOK], 0);
      end;{if}
    end;{if}
  {$IFEND}
  ...
  {$IF (not Defined(EX600)) or (not Defined(LIC600))}
    ...
    begin
      MessageDlg('PICheckSecurity : PISecure.pas'#13#13 +
      'The Plug-In (' + sDescription + ') has been compiled incorrectly'#13#13
      + 'You are compiling a v6/v7+ plug-in, but you have not got EX600 or LIC600 defined'
      , mtError, [mbOK], 0);
    end else
    begin
