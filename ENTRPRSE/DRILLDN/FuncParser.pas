...
// Parses the formula and returns True if it is considered to be a valid Exchequer formula
...
Begin { ParseFormula }
  ...
  If (FormulaLen > 0) Then Begin
    ...
    // Check it is an Exchequer function
    // HM 04/08/03: Always compare in uppercase to avoid case sensitivy issues
    ...
    Else
      DrillDownLog.AddString ('No Exchequer Function Found');
  End { If (FormulaLen > 0) }
  Else
    // Formula is blank
