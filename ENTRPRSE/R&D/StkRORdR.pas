...
uses
  ...
  ExtCtrls, StdCtrls, TEditVal, SBSPanel, bkgroup, Mask,ExWrap1U,VarSortV,
...
Function Stk_SuggROQ(StockR  :  StockRec;
  ...
  Procedure UpdateFromSortViewRecord(Back2Ord,
                                     GenWORQty,
                                     ResetMode
                                               :  Boolean;
                                     StkGrp    :  Str20;
  ...
  Procedure Exec_AutoNeed(Back2Ord,
                          GenWORQty,
                          ResetMode
                                    :  Boolean;
                          StkGrp    :  Str20;
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      If (IncWOR) or (StockR.DPackQty) then
...
{SS 13/04/2016 2016-R2
 ABSEXCH-17374 : Auto-set before sort view and after sort view. Only sorted stock
                 listed should generate POR.}

{ === Procedure to Update the records of Stock table based on the Sort view table   ========= }
...
begin
  Exec_AutoNeed(Back2Ord,GenWORQty,ResetMode,StkGrp,StkLocFilt,Fnum,FViewNum,KeyPath,KeyChk,MsgForm);
end;
...
Begin
  ...
  Try
    ...
    Status:=Find_Rec(B_GetGEq,F[FViewNum],FViewNum,RecPtr[FViewNum]^,KeyPath,KeyS);
    
    While (StatusOk) and (Ok2Go) and (CheckKey(KeyChk,KeyS,Length(KeyChk),BOff)) do
    begin
      // Check sortView is applied or not.
      if (FViewNum = SortTempF) and (FViewNum <> Fnum) then
      begin
        // Synchronize Stock record as per sort view applied Stock list.
        lStockKeyS := FullStockCode(SortTempRec.svtSourceDataStr);
        lStatus := Find_Rec(B_GetEq, F[Fnum], Fnum, RecPtr[StockF]^, StkCodeK, lStockKeyS);
      end;
      ...
      Begin
        ...
        Begin
          ...
          Stock_LocLinkSubst(TmpStk,StkLocFilt);
          ...
          If ((RoChk<>0) or ResetMode) and (Stk_InGroup(StkGrp,Stock)) then
          Begin
            ...
            Begin
                ...
                Begin
                  ...
                  Begin
                    ...
                    Begin
                      ...
                      RoDate:=EntSysObj.Stock.stReOrderDate;
                    end;
                  end;
                end;
              ...
              If (StatusOk) and (Not EmptyKey(StkLocFilt,LocKeyLen)) then
                // MH 26/10/2010 v6.5 ABSEXCH-9315: Modified call to stop creation of new stock-location records
                Update_LocROTake(Stock,StkLocFilt,0, Not ResetMode);
        ...
        Status:=Find_Rec(B_GetNext,F[FViewNum],FViewNum,RecPtr[FViewNum]^,KeyPath,KeyS);
      end;
    end; {While..}
  finally
    {$IFDEF CU}
...
Begin
  Exec_AutoNeed(Back2Ord,GenWORQty,ResetMode,StkGrp,StkLocFilt,Fnum,Fnum,KeyPath,KeyChk,MsgForm);
end; {Proc..}
...
Begin
  ...
  Begin
    ...
    //TG-01-06-2017- ABSEXCH-18684- Inactive CC/Dep gets display in 'Cost Centre/Department Search List'.
    FoundOk:=GetCust(Self,SuppTemp,FoundCode,BOff,2);
    ...
    With Cust do
      //PR: 02/02/2009 20080221084912 - Because last parameter (ProtectInp) was 1, GetProfileCCDep was always returning then
      //ROCCDep stored in the stock record.
      ROCCDep:=GetProfileCCDep(CustCC,CustDep,LStock.CCDep,ROCCDep,0);
//      ROCCDep:=GetProfileCCDep(CustCC,CustDep,LStock.CCDep,ROCCDep,1);
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        If (StatusOk) and (Not EmptyKey(StkLocFilt,LocKeyLen)) then
        begin
          //update the stock location record
          Update_LocROTake(LStock,StkLocFilt,0);
          //and disable the reorder flag on the stock record (so the record does not appear bold on the unfiltered list)
          ROFlg := False;
        end;
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        {$IFDEF MC_On}
          Flg:=(Flg or (ROCurrency<>Cust.Currency));
          UseCurr:=Cust.Currency;
        {$ENDIF}

        Calc_StockPrice(LStock,Cust,UseCurr,ROQty,Today, Rnum,RODisc,RODiscChr,StkLocFilt,Flg,0);


        If (Flg) or (ROCPrice=0) {$IFDEF MC_On} or (ROCurrency<>Cust.Currency) {$ENDIF} then {v5.52. Only update what was there if we have found something, or it was zero initially}
        Begin
