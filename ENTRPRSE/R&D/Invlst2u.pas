...
Procedure Delete_JobAct(Idr    :  IDetail);
 ...
 Uses
   ...
   {$IFDEF CU}
     Event1U,
     // CJS 16/06/2011 - ABSEXCH-11419 (Pack Qty Multiplier hook point for Freyssinet)
     CustWinU,
     CustInit,
     CustIntU,
   {$ENDIF}
...
Function Get_BudgMUp(JCode,
             ...
             :  With JobCtrl^.JobBudg do
                Begin
                  ...
                  Begin
                    ...
                    {$IFDEF EX601}
                        BrValue:=JBBCurrency_Txlate(BrValue,JBudgetCurr,0);
                    {$ENDIF}
  ...
  Var
    ...
    //PR: 16/06/2009 Procs to avoid rounding up actual cost
    procedure SetTempDecPlaces;
    begin
      TmpNetDec := Syss.NoNetDec;
      TmpCosDec := Syss.NoCosDec;
      Syss.NoNetDec := 12;
      Syss.NoCosDec := 12;
    end;

    procedure ResetDecPlaces;
    begin
      Syss.NoCosDec := TmpCosDec;
      Syss.NoNetDec := TmpNetDec;
    end;
        ...
        Begin
          ...
          Begin
            ...
            {$IFDEF SOP}
              ...
              // CJS 16/06/2011 - ABSEXCH-11419 (Pack Qty Multiplier hook point for Freyssinet)
              // CJS 15/07/2011 - ABSEXCH-11610 (Added check to ignore for non-stock lines)
              {$IFDEF CU}
              if CustomisationEnabled and (Trim(IdR.StockCode) <> '') then
              begin
                if (CustomHandlers.CheckHandlerStatus(WIMisc + 1, 7) = 1) then
                  Qty := Round_Up(Qty_OS(IdR) * IdR.QtyMul, Syss.NoQtyDec)
                else
            ...
            Begin
              ...
              begin
                //PR: 16/06/2009 DetLTotalND uses system dec places for rounding. This was causing issues with MBDs so
                //change to avoid rounding at this point.
                SetTempDecPlaces;
