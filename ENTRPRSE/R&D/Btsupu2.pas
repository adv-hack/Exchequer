...
Var
  ...
  STDCurrList,
  STDAccStatusList,
  STDAccLedStatusList :  TStringList; //PL 04/01/2017 : 2017 R1 :	ABSEXCH-16605 Custom Transaction Matching chops maCustomRef off at 10 characters instead of 20
...
Function TxLate_NOMClass(CLItem  :  Integer;
  ...
  Procedure Set_DefaultAccStatus(ThisSet  :  TStrings;
                                 ShowInc,
                                 ShowFull :  Boolean); //HV 19/04/2016 2016-R2 ABSEXCH-9497: Drop down list be added when Status is selected to avoid mis-typing the three valid options
...
{$IFDEF STK}
  Procedure Set_DefaultStkT(ThisSet  :  TStrings;
                            ShowInc,
                            ShowFull :  Boolean;
                            SortViewList : Boolean=False); //HV 12/04/2016 2016-R2 ABSEXCH-9505: Stock List Sort Views - add the ability to sort by stock type with dropdown
...
Procedure Init_STDAccStatusList; //HV 19/04/2016 2016-R2 ABSEXCH-9497: Drop down list be added when Status is selected to avoid mis-typing the three valid options
Procedure Init_STDAccLedStatusList; //PL 04/01/2017 2017-R1 ABSEXCH-17809 : Default list values for Status field in Sort View of Trader > Ledger is not correct.
{$IFDEF STK}
...
Begin
  ...
  Begin
    ...
    (*ThisSet.Add('M - Manual');*)          // SSK 23/06/2017 2017-R1 ABSEXCH-13530 : changed to remove vatcode 'M' from the list
  end;
...
Begin
  ...
  Begin
    ...
    (*If (ShowMan) then                   // SSK 23/06/2017 2017-R1 ABSEXCH-13530 : changed to remove vatcode 'M' from the list
...
//HV 11/04/2016 2016-R2 ABSEXCH-15408: Added new parameter to remove blank transaction currency
Procedure Set_DefaultCurr(ThisSet  :  TStrings;
                          IncZero,
                          ShowFull :  Boolean;
...
Begin
  ...
  //HV 11/04/2016 2016-R2 ABSEXCH-15408: Added new parameter to remove blank transaction currency 
  if not ShowBlankRecord then
  begin
    for n:=Pred(ThisSet.Count) downto 0 do
    begin
      if (Trim(ThisSet[n]) = '-') or (ThisSet[n] = '') then
        ThisSet.Delete(n);
    end;
  end;
...
{* Procedure to Return TList of Cust/Supp Account Status*}
  Procedure Init_STDAccStatusList;
  Var
    n  :  Byte;
  Begin
    STDAccStatusList:=TStringList.Create;
    try
      With STDAccStatusList do
      Begin
        Add('Open');
        Add('See Notes');
        Add('On Hold');
  ...
  Begin
    ...
    try
      ...
      Begin
        ...
        Add('Suspend');
        Add('PPD Available'); //PL 06/01/2017 2017-R1 ABSEXCH-17809 changed these two options strings
        Add('PPD Taken');
      end; {With..}
    except
...
{$IFDEF STK}
  ...
  Begin
    ...
    if SortViewList then
    begin
      n := ThisSet.IndexOf('Group');
      if n <> -1 then
        ThisSet.Delete(n);
      n := ThisSet.IndexOf('Discontinued');
      if n <> -1 then
        ThisSet.Delete(n);
    end
  end;
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      //PR: 13/07/2010 fix for problem where over-running a line with one word would move the entire line to the next line.
      if (wp > 1) and (Text[Length(Text)] <> ' ') then
      begin
      ...
      Begin
        ...
        //PR: 14/07/2010 Fix for issue where wrapping text to the next line would delete the contents of that line

        //If no contents of next line then simply move the word to next line
        if Trim(TxC.Text) = '' then
          TxC.Text:=lw+Key
        else //Next line has text -  check if we have room to insert wrapped text in front
        if Length(TxC.Text + lw + Key) < TxC.MaxLength - 1 then
          TxC.Text:=lw+Key + '  ' + TxC.Text
        else
        begin //no room to insert - leave as is and let the user do it manually
        
          Text :=OldText;
          GoToNextField := False;
        end;

        if GoToNextField then
        begin
        If (TxC.CanFocus) then
          TxC.SetFocus;

          TxC.SelStart:=Succ(Length(lw));
...
{$ENDIF}
  ...
  { === Procedure to add CIS2 page === }

  Procedure Check572Sys;


  Const
    SysChk  :  Array[0..1] of SysrecTypes = (CISR,CIS340R);
...
Begin
  ...
  STDNomTList:=Nil;
  STDCurrList:=Nil;
  STDAccStatusList:=Nil;
  STDAccLedStatusList:=Nil; //PL 04/01/2017 2017-R1 ABSEXCH-17809 : Default list values for Status field in Sort View of Trader > Ledger is not correct.

  //RB 06/07/2017 2017-R2 ABSEXCH-18944: Use Global SQL Connection for SQLCaller
  {$IFDEF EXSQL}
  If SQLUtils.UsingSQL Then
  begin
    if Not Assigned(GlobalAdoConnection) then
  ...
  Begin
    ...
    Init_STDNomTList;
    Init_STDAccStatusList; //HV 19/04/2016 2016-R2 ABSEXCH-9497: Drop down list be added when Status is selected to avoid mis-typing the three valid options
    Init_STDAccLedStatusList; //PL 04/01/2017 2017-R1 ABSEXCH-17809 : Default list values for Status field in Sort View of Trader > Ledger is not correct.
    {$IFDEF STK}
      Init_STDStkTList;
      Init_STDValList;
      Init_STDQBList;
...
Begin
  ...
  //HV 19/04/2016 2016-R2 ABSEXCH-9497: Drop down list be added when Status is selected to avoid mis-typing the three valid options
  If (Assigned(STDAccStatusList)) then
  Begin
    STDAccStatusList.Free;
    STDAccStatusList:=Nil;
  end;
  ...
  //PL 04/01/2017 2017-R1 ABSEXCH-17809 : Default list values for Status field in Sort View of Trader > Ledger is not correct.
  If (Assigned(STDAccLedStatusList)) then
  Begin
    STDAccLedStatusList.Free;
    STDAccLedStatusList:=Nil;
  end;
