               ...
               Begin
                 ...
                 Begin
                   ...
                   Begin
                     ...
                     {$B-}
                     If (((EmptyKey(MToCode,StkKeyLen)) or (LGetMainRecPos(StockF,FullStockCode(MToCode)) and (Stock.StockType=StkGrpCode)) and (MToCode<>MFromCode)) or (Not EmptyKey(Trim(NewStkCode),StkKeyLen)))  then
                     {$B+}
                     Begin
                       With MoveRepCtrl^ do
                       Begin
    ...
    Begin
      ...
      Repeat
        ...
        Begin
          ...
          ShowStatus(2,'Updating discount matricies.');
    ...
    Begin
      ...
      Begin
        ...
        Repeat
          ...
          If (Not EmptyKey(MoveStk.JobCat,JobCodeLen)) then {* Do not process if on level 0 *}
          Begin
                ...
                Begin
                  ...
                  Begin
                    ...
                    Begin
                      ...
                      Move_PostCtrl(MoveStk.JobCat,HPCode,
                                    (Purchases*DCnst),
                                    (Sales*DCnst),
                                    (Cleared*DCnst),
                                    Cr,Yr,Pr,
...
Begin
  ...
  Begin
    ...
    If (MyOHandle>0) then {* Added to Q ok *}
      SendMessage(MyOHandle,WM_FormCloseMsg,70+Ord(MAutoRecover),LongInt(@SendRecCtrl^));
...
Begin
  ...
  If (Not HaveAborted) and (MyOHandle>0) and (fWhoAmI In [0]) then
    SendMessage(MyOHandle,WM_FormCloseMsg,72,LongInt(@SendRecCtrl^));
   ...
   {$IFDEF EXSQL}
     ...
     Begin
       If (tViewNo <> 0) then
         ...
         Res := ResetViewHistory(SetDrive, tViewNo)
       Else
         // Zero down all Nominal Views
         Res := ResetViewHistory(SetDrive, 0);
       If (Res < 0) Then
    ...
    Begin
      Begin
        ...
        Begin
          ...
          Begin
            If (GetHist.ExClass In YTDSet) then
              YTDType:=YTD
            else
              YTDType:=YTDNCF;
...
Begin
  ...
  Begin
    ...
    Try
      ...
      LStatus := 0;
      LNomView^ := PostView;
      While (LStatus = 0) And (LNomView^.NomViewLine.ViewCat <> 0) Do
      Begin
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        With PostView.NomViewLine do
          If (Level=0) then
            FiltOk:=(CheckKey(LinkCCDep[BOn],CCDep[BOn],Length(LinkCCDep[BOn]),BOff) and CheckKey(LinkCCDep[BOn],CCDep[BOn],Length(LinkCCDep[BOn]),BOff))
          else
            FiltOk:=BOn;
        ...
        Repeat
          ...
          Begin
            ...
            Begin
              If (HedHist.ExClass In YTDSet) then
                YTDType:=YTD
              else
                YTDType:=YTDNCF;
...
Const
  Clearwhich  :  Array[0..6] of Str20 = ('the Cleared','the Actual','the G/L Heading','every Actual','','the Job Tree Group','the G/L views');
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        If (Syss.AuditYr<>0) and (SBSIn) then
        Begin
          mbRet:=CustomDlg(Application.MainForm,'Warning!','This data has been purged',
  ...
  try
    ...
    Begin
      ...
      WMsg:=ConCat('You are about to change Stock Code ',dbFormatName(LStock.StockCode,LStock.Desc[1]),
                   ' to Stock Code ',Trim(NS),#13);
      WMsg:=ConCat(WMsg,'This operation should be performed with all other users logged out.  A backup ',
            'MUST be taken since the integrity of the Stock Tree will be damaged should the change fail.',#13,
            'Do you wish to continue?');

      mbRet:=CustomDlg(Application.MainForm,'Please Confirm','Stock Code Renumber',WMsg,mtConfirmation,[mbYes,mbNo]);
  ...
  try
    ...
    Begin
      ...
      WMsg:=ConCat('You are about to change Account Code ',dbFormatName(LCust.CustCode,LCust.Company),
                   ' to Account Code ',Trim(NS),#13);
      WMsg:=ConCat(WMsg,'This operation should be performed with all other users logged out.  A backup ',
            'MUST be taken since the integrity of the Account Ledger will be damaged should the change fail.',#13,
            'Do you wish to continue?');

      mbRet:=CustomDlg(Application.MainForm,'Please Confirm','Account Code Renumber',WMsg,mtConfirmation,[mbYes,mbNo]);
...
Begin
  Result:=Confirm_ChangeAccCode(LCust,NS,AOwner);
end;
