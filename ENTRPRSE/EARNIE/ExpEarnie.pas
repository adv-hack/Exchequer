...
uses UseDllU,
     ...
     sysutils,CmnEarnie32,contnrs,classes,forms,logFileRtn;
{$I ExDllBt.inc}
{$I ExchDll.inc}
...
Begin
  ...
  try             //global rate returned if empcode parameter is blank
    ...
    begin
      ...
      begin
        if (JobRateRec.PayRate In [1..10]) then
        begin
...
begin
  try
    ...
    if (Exp1.EmpCode + inttostr(Exp1.Rate)) < (Exp2.EmpCode + IntToStr(Exp2.Rate)) then result := -1;
    if (Exp1.EmpCode + IntToStr(Exp1.Rate)) > (Exp2.EmpCode + IntToStr(Exp2.Rate)) then result := 1;
    if (Exp1.EmpCode + intToStr(Exp1.Rate)) = (Exp2.EmpCode + IntToStr(Exp2.Rate)) then result := 0;
      ...
      begin
        case ExportType of
          ...
          begin
            ...
            Writeln(TxtFile,(rate):2,noHour:10:2);

            Write(TxtFile,EmpCode);
            Write(TxtFile,StringOfChar(' ',10-length(EmpCode)));
            Writeln(TxtFile,Rate:2,((Rate)*100.00):10:2);
          end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              for Counter := 1 to TranRec.LineCount do
              begin
