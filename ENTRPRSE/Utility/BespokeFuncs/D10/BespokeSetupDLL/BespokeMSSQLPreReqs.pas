...
Type
  ...
  TPreReqIndex = (priWinInstaller=0, priNetFramework=1, priSQLExpress=2{, priLicencing=5, priPSQLWGE=6});
  ...
  Private
    ...
    // Checks for the presence of the Windows Installer v4.5
    Procedure CheckForWinInst45(bAssumeInstallWorked : boolean = FALSE);
...
//    Procedure CheckForMDAC28;
    ...
    Procedure CheckForNetFramework(bAssumeInstallWorked : boolean = FALSE);
...
//    Procedure CheckForIRISLicencing;
    ...
    Procedure CheckForSQLExpress(bAssumeInstallWorked : boolean = FALSE);
...
//    Procedure CheckForPervasive;
  ...
  Public
    ...
    Procedure CheckPreReqs(iClicked : integer);
  End; // TPreRequisites
...
Procedure TPreRequisites.CheckPreReqs(iClicked : integer);
Var
...
Begin // CheckPreReqs
  ...
  if iClicked = Ord(priWinInstaller)+1 then CheckForWinInst45(TRUE)
  else CheckForWinInst45;

  // MS XML 4.0 Parser
//  CheckForMSXML4;

  // .NET Framework 3.5
  if iClicked = Ord(priNetFramework)+1 then CheckForNetFramework(TRUE)
  else CheckForNetFramework;
...
//  If (FPreReqCheckType <> prcExchDSR) Then
  if iClicked = Ord(priSQLExpress)+1 then CheckForSQLExpress(TRUE)
  else CheckForSQLExpress;
...
Procedure TPreRequisites.CheckForWinInst45(bAssumeInstallWorked : boolean = FALSE);
Var
  sPath : ANSIString;
  V1, V2, V3, V4: Word;
  oOSChecks : TOSChecks;
...
Begin // CheckForWinInst45
  ...
  {        If  (V1 > 3) Or
        ...
        If DebugMode Then
        Begin
          If (MessageDlg('Windows Installer 4.5 Installed', mtConfirmation, [mbYes, mbNo], 0) = mrYes) Then
            FPreReqChecks[priWinInstaller] := prsInstalled
          Else
            FPreReqChecks[priWinInstaller] := prsMissing;
        End; // If DebugMode
      End // If (Not (oOSChecks.ocWindowsVersion In [wv2003Server, wv2003TerminalServer, wvVista]))
      Else
...
// Checks for the presence of the .NET Framework v2.0
Procedure TPreRequisites.CheckForNetFramework(bAssumeInstallWorked : boolean = FALSE);
Begin // CheckForNetFramework
  ...
  begin
    If DotNetInfo.Net35Installed Then
      FPreReqChecks[priNetFramework] := prsInstalled;

    If DebugMode Then
    Begin
      If (MessageDlg('.NET Framework v3.5 Installed', mtConfirmation, [mbYes, mbNo], 0) = mrYes) Then
        FPreReqChecks[priNetFramework] := prsInstalled
      Else
...
Procedure TPreRequisites.CheckForSQLExpress(bAssumeInstallWorked : boolean = FALSE);
Var
  oReg : TRegistry;
Begin // CheckForSQLExpress
  ...
  begin
    ...
    Try
      ...
      Begin
        If oReg.ValueExists('IRISSOFTWARE')
        or oReg.ValueExists('IRISEXCHEQUER') Then
          FPreReqChecks[priSQLExpress] := prsInstalled;
...
(*  If (FPreReqCheckType = prcNetwork) Then
    Result := (FPreReqChecks[priWinInstaller] = prsInstalled) And       // Windows Installer 4.5
//              (FPreReqChecks[priMSXML40] = prsInstalled) And            // MS XML 4.0
//              (FPreReqChecks[priMDAC] = prsInstalled) And               // MDAC 2.8 SP1
              (FPreReqChecks[priNetFramework] = prsInstalled) And       // .NET Framework 3.5
...
// MH 12/09/08: Don't check for the Dashboard running under Exchequer
//              (FPreReqChecks[priSQLExpress] = prsInstalled) And         // SQL Express 2005 - IRISSOFTWARE named instance
              (FPreReqChecks[priNetFramework] = prsInstalled)           // .NET Framework 3.5
  Else
    Raise Exception.Create('TPreRequisites.GetAllChecksPassed: Unhandled CheckType');*)



//  Result := (FPreReqChecks[priWinInstaller] = prsInstalled) // Windows Installer 4.5
//  and (FPreReqChecks[priNetFramework] = prsInstalled) // .NET Framework 3.5
//  and (FPreReqChecks[priSQLExpress] = prsInstalled); // SQL Express 2005 - IRISSOFTWARE named instance
