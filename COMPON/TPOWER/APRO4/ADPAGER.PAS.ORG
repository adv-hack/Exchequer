  ...
  var
    ...
    begin
      ...
      { Make BlindDial prefix }
        ...
        FPort.Output := S + FPhoneNumber + atpCRLF;
      end else
      begin
      { Normal prefix dial }
        FPort.Output := FDialPrefix + FPhoneNumber + atpCRLF;
      end;
    end;
  begin
    ...
    if Assigned(FPort) and FPort.Open then
      FPort.Output := FModemInit + atpCRLF;
...
begin
  ...
  do begin
    ...
    if FCancelled or FAborted then
      ...
      if (FDialAttempt < FDialAttempts) then                             {!!.02}
...
procedure TApdCustomModemPager.TerminatePage;                            {!!.02}
// This procedure is called when not using TAPI
var                                                                      {!!.02}
  WaitTimer: EventTimer;                                                 {!!.02}
  I, res : integer;                                                      {!!.02}
  TheCommand : String;                                                   {!!.02}
begin                                                                    {!!.02}
  if DirectToPort or not FPort.Open then                                 {!!.02}
    exit;                                                                {!!.02}
  OKTrig := FPort.AddDataTrigger(FapOKTrig, True);                       {!!.02}
    ...
    TheCommand := '';                                                    {!!.02}
      ...
      for I := 1 to Length(adpgDefModemHangupCmd) do                     {!!.02}
        if adpgDefModemHangupCmd[I] = '~' then                           {!!.02}
          DelayTicks(2, True)                                            {!!.02}
        else begin                                                       {!!.02}
          if FPort.DCD then // Send +++ to escape - ATH to hangup        {!!.02}
            TheCommand := TheCommand + adpgDefModemHangupCmd[I]          {!!.02}
          else begin                                                     {!!.02}
            FreeTrigger(FPort, OKTrig, FapOKTrig);                       {!!.02}
            exit;                                                        {!!.02}
          end;                                                           {!!.02}
          if Length(TheCommand) = 3 then begin                           {!!.02}
            FPort.Output := TheCommand;                                  {!!.02}
            TheCommand := '';                                            {!!.02}
            if FPort.DCD then begin                                      {!!.02}
              NewTimer(WaitTimer, 36);                                   {!!.02}
              repeat                                                     {!!.02}
                Res := SafeYield;                                        {!!.02}
              until mpGotOkay or FAborted or FCancelled or               {!!.02}
                    TimerExpired(WaitTimer) or (Res = wm_Quit)           {!!.02}
            end else begin                                               {!!.02}
              FreeTrigger(FPort, OKTrig, FapOKTrig);                     {!!.02}
              exit;                                                      {!!.02}
            end;                                                         {!!.02}
...
procedure TApdCustomModemPager.Send;
begin
