...
uses
  Classes, Dialogs, Variants, uExDatasets, uBtrv, SysUtils;
...
implementation
uses
  {$IFDEF ENTER1}SQLUtils,{$ENDIF ENTER1} APIUtil, MathUtil; //PR: SQLUtils needed for bodge in LoadDataset function below.
...
//    SetBounds;
    ...
    if Result <> 0 then
    begin
      MsgBox('The following Btrieve error occurred when opening the file (' + fFileName
      + ') for this list :'#13#13'Error : ' + IntToStr(Result), mtError,[mbOK], mbOK
      , 'TBtrieveDataset.OpenData');
    end;{if}
...
//***************** Bodge Alert!! ****************************//
//PR: Added temporary change to compensate for issue where Emulator is returning the wrong datatype
//for floating point indexes. Used for Bank Rec Wizard Amount column only hence unusual SortKey!
{$IFDEF ENTER1} //PR: 'ENTER1'  is a new define which MH has added to the build routines to identify Enter1.
if SQLUtils.UsingSQL and (SortKey = 'Modigliani') then
begin
  fBoundHigh := fSearchKey + #133#235#255#31#95#160#2#66;   //Needed to change this value to avoid emulator crash
  fBoundLow := fSearchKey + #0#200#78#103#109#193#171#195;  //Same as value for pervasive
end;
{$ENDIF ENTER1}
//***************** End Bodge  ******************************//
...
begin
  ...
  if fDataRecord = nil then result := -1
  else begin
...
function TBtrieveDataset.Field({DatasetID: integer;} FieldName: string; ForDisplay: Boolean = False): variant;
var
ColIndex: integer;
begin
  Result:= '';
...
{  if (VarType(fDataset) <> varArray)
  and (VarType(fDataset[fCursor]) <> varArray) then exit;}
...
{      if (Selected >= 0)  and Assigned(OnSelectRecord)
      then OnSelectRecord(Self, SelectType, RecordNo, fDataRecord);}
    end;
//  end;
end;
...
var
  ...
  function AltColtSeqDefined : boolean;
  begin
    Result := TRUE;

    // if no of segs is a fractional number then AltColtSeq cannot have been Defined
    if not ZeroFloat((Round((iRecLength - SizeOf(FileStatSpec) - SizeOf(AltColtSeq)) / SizeOf(KeySpec))) -
    (iRecLength - SizeOf(FileStatSpec) - SizeOf(AltColtSeq)) / SizeOf(KeySpec))
    then Result := FALSE;

    // if no of segs is 0 or less then AltColtSeq cannot have been Defined
    if Round((iRecLength - SizeOf(FileStatSpec) - SizeOf(AltColtSeq)) / SizeOf(KeySpec)) <= 0
    then Result := FALSE;
  end;{AltColtSeqDefined}
...
//    result := fSearchKey + #255#255#255#127 + StringOfChar(#255,255);
  ...
  if iStatus = 0 then
  begin
    if AltColtSeqDefined
    then iNoOfSegs := Round((iRecLength - SizeOf(FileStatSpec) - SizeOf(AltColtSeq)) / SizeOf(KeySpec))
    else iNoOfSegs := Round((iRecLength - SizeOf(FileStatSpec)) / SizeOf(KeySpec));
