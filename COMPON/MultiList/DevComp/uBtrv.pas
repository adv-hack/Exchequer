...
Const
  ...
  // CJS 2012-02-07 - ABSEXCH-12402
  // Modified for compatibility with plugins which are run on a site using
  // Exchequer SQL, but where the plugin uses Btrieve files. The plugin must
  // be compiled with the EXBTRVPLUGIN directive defined.
  {$IFDEF EXSQL}
    {$IFDEF EXBTRVPLUGIN}
...
Begin
  ...
  Repeat
    ...
    If Not (Trans In [84,85]) then Status_Means(Trans);
...
Begin
  ...
  {$IFNDEF BCS}
    ...
    If (TmpBo) then  {* Its C/S if >v5, or BT type = 'S(erver *}
      ...
      Chk4BCS:=false; {* Force CS msg off, as Bt not loaded anyway...*}
...
{$IFNDEF NOBTRegChk}
  ...
  Begin
    BTForceLocalEngine :=false;

    BTForceCSEngine:=false;
