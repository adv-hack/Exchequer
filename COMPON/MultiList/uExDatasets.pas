...
//    function FreeDataset(DatasetID: integer): integer; virtual; abstract;
  ...
  public
    ...
    function Field({DatasetID: integer;} FieldName: string; ForDisplay: Boolean = False): variant; virtual; abstract;
    function FreeDataset(DatasetID: integer): integer; virtual; abstract;
    property Open: boolean read fOpen write SetOpen;

    // CJS 04/12/2008 - Allowed writing to the PrimaryKey
...
begin
  if (DatasetID >= Low(fRecCount)) and (DatasetID <= High(fRecCount))
  then fRecCount[DatasetID]:= NewRecCount;
end;
