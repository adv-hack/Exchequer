...
uses
  ...
  , FileUtil, GfxUtil, StrUtil, APIUtil, BTConst, VirtualTrees, GlobVar;
...
type
  ...
  public
    //PR: 09/11/2017 ABSEXCH-19303 Made CloseFile public to allow use when
    //running Pervasive Data Encryption from MCM
    procedure CloseFile;
    constructor Create;
    destructor Destroy; override;
    //PR: 09/11/2017 ABSEXCH-19303 Added default ForceReopen to allow use when
    //running Pervasive Data Encryption from MCM
    procedure ReopenFile(ForceReopen : Boolean = False);
    procedure ExportBtrieveFileTo(sFileName : string);
    procedure SaveList(TheList : TMultiList; sFormName : string); OverLoad;
    procedure SaveList(TheList : TVirtualStringTree; sFormName : string); OverLoad;
    procedure LoadList(TheList : TMultiList; sFormName : string); OverLoad;
    procedure LoadList(TheList : TVirtualStringTree;  sFormName : string); OverLoad;
...
//    procedure SaveForm(TheForm : TForm); overload;
    ...
    procedure RestoreListDefaults(TheList : TMultiList; sFormName : string); overLoad;
    procedure RestoreListDefaults(TheList : TVirtualStringTree; sFormName : string); overLoad;
...
//    property LocalDir : string read fLocalDir write SetLocalDir;
  ...
  // MH 29/10/2010 ABSEXCH-10427: Free's the oSettings object if in existance
...
type
  ...
  // Note: Structure duplicated in Entprse\Conversion\v70 SQL Converter\Converter\DataWriteObjs\oSettingsDataWrite.Pas
...
Procedure FreeSettings;
Begin // FreeSettings
  If Assigned(oLSettings) Then
  Begin
    oLSettings.Free;
    oLSettings := NIL;
  End; // If Assigned(oLSettings)
End; // FreeSettings
...
{$ENDIF EXSQL}
    ...
    if iOpenStatus = 0 then begin  //PR: 15/02/2018 ABSEXCH-19303 Added owner for opening file
      iOpenStatus := BTOpenFile(fFileVar, sMiscDirLocation + FILENAME, 0, @CIDRec, ExBTOWNER);
      BTShowError(iOpenStatus, 'BTOpenFile', sMiscDirLocation + FILENAME);
    end;{if}
  end;{OpenBTFile}

begin
  // NF: 25/07/2008 - Added Client ID Support so that this works properly under SQL
  BTPrimeClientIdRec(CIDRec, 'SE', 99);
...
procedure ListRec2VirtualStringTree(TheListSettingsRec : TListSettingsRec; TheList: TVirtualStringTree; bMoveColumns : boolean = TRUE);
var
  iCol2, iColumn : integer;
  iTempPos : Byte;
begin{ListRec2List}
  FontRec2Font(TheListSettingsRec.lsMainFont, TheList.Font);
  For iColumn := 0 to TheList.Header.Columns.Count -1 do
  begin
    if bMoveColumns then
    begin
      // assign column widths
      TheList.Header.Columns[iColumn].Width := TheListSettingsRec.lsColumnWidths[iColumn];
      TheList.Header.Columns[iColumn].Position := TheListSettingsRec.lsColumnOrder[iColumn];
    end;{if}
  end;{for}
  TheList.Refresh;
end;{ListRec2List}
...
{HV 11/03/2016 2016-R2 ABSEXCH-13588: Provided support
      to VirtualStringTree for saving co-ordinates. }
procedure VirtualStringTree2ListRec(TheList: TVirtualStringTree; var TheListSettingsRec : TListSettingsRec; bMoveColumns : boolean = TRUE);
var
  iColumn : integer;
begin{List2ListRec}
  FillChar(TheListSettingsRec,SizeOf(TheListSettingsRec),#0);
  with TheListSettingsRec do
  begin
    Font2FontRec(TheList.Font, TheListSettingsRec.lsMainFont);
    // get column properties
    for iColumn := 0 to TheList.Header.Columns.Count -1 do
    begin
      if bMoveColumns then lsColumnWidths[iColumn] := TheList.Header.Columns[iColumn].Width;
      if bMoveColumns then lsColumnOrder[iColumn] := TheList.Header.Columns[iColumn].Position;
    end;{for}
  end;{with}
end;{VirtualStringTree2ListRec}
...
//PR: 09/11/2017 ABSEXCH-19303 Added default ForceReopen to allow use when
//running Pervasive Data Encryption from MCM
procedure TFormSettings.ReopenFile(ForceReopen : Boolean = False);
...
begin
{$IFDEF EXSQL}
  if SQLUtils.UsingSQL or ForceReopen then
  begin
    iStatus := BTOpenFile(fFileVar, sMiscDirLocation + FILENAME, 0, @CIDRec, ExBTOwner);
...
{$ENDIF}
 ...
 {HV 11/03/2016 2016-R2 ABSEXCH-13588: Provided support
      to VirtualStringTree for saving co-ordinates. }
procedure TFormSettings.SaveList(TheList: TVirtualStringTree;
  sFormName: string);
var
  KeyS : Str255;
  iStatus : integer;
...
begin{SaveList}
  ...
  iStatus := BTFindRecord(B_GetEq, fFileVar, SettingsRec, fBufferSize, fsLookupIdx, KeyS, @CIDRec);
  InitialiseRec(SettingsRec, FS_LIST, sLookUp);
  VirtualStringTree2ListRec(TheList, SettingsRec.fsList);
  if iStatus = 0 then
  begin
    iStatus := BTUpdateRecord(fFileVar, SettingsRec, fBufferSize, fsLookupIdx, KeyS, @CIDRec);
    BTShowError(iStatus, 'BTUpdateRecord', sMiscDirLocation + FILENAME);
...
{HV 11/03/2016 2016-R2 ABSEXCH-13588: Provided support
      to VirtualStringTree for saving co-ordinates. }
procedure TFormSettings.LoadList(TheList: TVirtualStringTree;
  sFormName: string);
var
  KeyS : Str255;
  iStatus : integer;
...
begin{LoadList}
  ...
  KeyS := FS_LIST + sLookUp;
  iStatus := BTFindRecord(B_GetEq, fFileVar, SettingsRec, fBufferSize, fsLookupIdx, KeyS, @CIDRec);
  if iStatus = 0 then ListRec2VirtualStringTree(SettingsRec.fsList, TheList)
  else begin
    if not (iStatus in [4, 9]) then BTShowError(iStatus, 'BTFindRecord', sMiscDirLocation + FILENAME);
  end;{if}
...
procedure TFormSettings.RestoreListDefaults(TheList: TVirtualStringTree;
  sFormName: string);
var
  KeyS : Str255;
  iStatus : integer;
