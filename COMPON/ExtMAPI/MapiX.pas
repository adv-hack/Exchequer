...
type
  ...
  {To minimise use of IfDefs through the MAPI code, redefine PChar.
   In D10, PChar is defined as PWideChar, whereas in D6 it is PAnsiChar.

   In D6 LargeInt and LargeUInt are both defined as Int64, so LargeInt can be
   passed as a var parameter to a function that expects a LargeUInt. In D10,
   LargeUInt is defined as UInt64 so using LargeInt in that situation won't
   compile.}
  ...
  IMAPISession =
    ...
    [strIID_IMAPISession]
      ...
      function OpenMsgStore (ulUIParam : ULONG; cbEntryID : ULONG;
        lpEntryID : PENTRYID; const lpInterface : PIID; ulFlags : ULONG;
      ...
      function OpenEntry (cbEntryID : ULONG; lpEntryID : PENTRYID;
        ...
        lppUnk : PUnknown) : HResult; stdcall;
...
function MAPIAllocateBuffer (cbSize : ULONG;
  lppBuffer : PPointer) : SCODE; stdcall;
...
function LoadExtendedMAPIFuncs : Boolean;

var
  _MAPILogonEx : Function(ulUIParam : ULONG; lpszProfileName : PChar;
                        lpszPassword : PChar; ulFlags : ULONG; {  ulFlags takes all that SimpleMAPI does + MAPI_UNICODE }
                        out lppSession : IMAPISession) : HResult; stdcall;

  _MAPIAllocateBuffer : Function(cbSize : ULONG;lppBuffer : PPointer) : SCODE; stdcall;

  _MAPIAllocateMore : Function(cbSize : ULONG; lpObject : Pointer; var lppBuffer : Pointer) : SCODE; stdcall;

  _MAPIFreeBuffer : Function(lpBuffer : Pointer) : ULONG; stdcall;

  _MAPIAdminProfiles : Function(ulFlags : ULONG; out lppProfAdmin : IProfAdmin) : HResult; stdcall;

  _MAPIInitialize : function(lpMapiInit : Pointer) : HResult; stdcall;
  _MAPIUninitialize : Procedure; stdcall;
...
begin
  ...
  @_MAPILogonEx        := nil;
  @_MAPIAllocateBuffer := nil;
  @_MAPIAllocateMore   := nil;
  @_MAPIFreeBuffer     := nil;
  @_MAPIAdminProfiles  := nil;
  @_MAPIInitialize     := nil;
  @_MAPIUninitialize   := nil;

  DLLHandle := LoadLibrary(Mapi32Dll);
  if DllHandle > 0 then
  begin
    @_MAPILogonEx        := GetProcAddress(DLLHandle, 'MAPILogonEx');
    @_MAPIAllocateBuffer := GetProcAddress(DLLHandle, 'MAPIAllocateBuffer');
    @_MAPIAllocateMore   := GetProcAddress(DLLHandle, 'MAPIAllocateMore');
    @_MAPIFreeBuffer     := GetProcAddress(DLLHandle, 'MAPIFreeBuffer');
    @_MAPIAdminProfiles  := GetProcAddress(DLLHandle, 'MAPIAdminProfiles');
    @_MAPIInitialize     := GetProcAddress(DLLHandle, 'MAPIInitialize');
    @_MAPIUninitialize   := GetProcAddress(DLLHandle, 'MAPIUninitialize');
   end;

    Result := Not(
          (@_MAPILogonEx        = nil)
       or (@_MAPIAllocateBuffer = nil)
       or (@_MAPIAllocateMore   = nil)
       or (@_MAPIFreeBuffer     = nil)
       or (@_MAPIAdminProfiles  = nil)
       or (@_MAPIInitialize     = nil)
       or (@_MAPIUninitialize   = nil)
       );
end;
