...
var tid : dword;
begin
  CloseHandle(dword(BeginThread(nil, 0, BeginThreadFunc, nil, 0, tid)));
end;
...
begin
  ...
  FillRichEdit(Explanation, CWelcomeExplanation);
  FillRichEdit(SourceCode, CWelcomeSourceCode);
end;
...
begin
  ...
  if btn <> nil then begin
    ...
    Explanation.Text := CExplanations[btn.Tag];
    FillRichEdit(SourceCode, CSourceCodes[btn.Tag]);
  end;
end;
