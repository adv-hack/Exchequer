...
begin
  with project do begin
    ...
    SetLength(Forms, 3);
    {$ifdef CodeJak}
      Forms[0] := 'TPF0' + #$0e + 'TCJContactForm' + #$0b + 'ContactForm' +
                           #$07 + 'Message'        + #$0c + #$13#$00#$00#$00 + 'Contact Information' +
                           #$08 + 'OnAction'       + #$0c + #$1d#$00#$00#$00 + 'iBmadExcept.HandleContactForm' +
    {$else}
    ...
    {$endif}
                ...
                #$00;
    {$ifdef CodeJak}
      Forms[1] := 'TPF0' + #$0e + 'TCJDetailsForm' + #$0b + 'DetailsForm' +
    {$else}
...
begin
  ...
  {$ifdef CodeJak}
    if RegOpenKeyEx(HKEY_LOCAL_MACHINE, 'Software\iBiSS\CodeJak\madExcept', 0, KEY_QUERY_VALUE, hk) = 0 then begin
  {$else}
  ...
  {$ifdef CodeJak}
    if RegOpenKeyEx(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\madExcept', 0, KEY_QUERY_VALUE, hk) = 0 then begin
  {$else}
...
begin
  ...
  {$ifdef CodeJak}
    result := RegOpenKeyEx(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\madExcept', 0, KEY_QUERY_VALUE, hk) = 0;
  {$else}
...
begin
  ...
  {$ifdef CodeJak}
    c1 := RegCreateKeyEx(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\madExcept', 0, nil, 0, KEY_ALL_ACCESS, nil, hk, @c1);
  {$else}
...
{  function Align(value, align: dword) : dword;
  ...
  begin
    {$ifdef CodeJak}
      result := 'TPF0' + #$8 + 'TCodeJak' + #$9 + 'iBmadExcept';
    {$else}
    ...
    with project, mapFile do 
      for i1 := 0 to high(CMapInfos) do
        with CMapInfos[i1] do begin
          {$ifdef CodeJak}
            c1 := FindPublic(false, cMadPublicName, var_);
            logMapEntry(c1, 'iBmadExcept.' + var_);
          {$else}
...
begin
          ...
          if IsValid then begin
            ...
            if c1 <> INVALID_HANDLE_VALUE then begin
              ...
              if boc <> 0 then begin
                SetLength(s1, 20);
                {$ifdef CodeJak}
                  TPACardinal(s1)^[0] := FindPublic(true, CMadPublicName, 'CalibrateCode');
                  TPACardinal(s1)^[1] := FindPublic(false, CMadPublicName, 'CalibrateData');
                {$else}
  ...
  //            if Enabled then begin
                ...
                {$ifdef CodeJak}
                  UpdateResourceW(c1, PWideChar(RT_RCDATA), 'TCODEJAK', 0, pointer(s1), Length(s1));
                {$else}
                      ...
                      if i2 > 0 then begin
                        ...
                        if s3 <> '' then begin
                          ...
                          if s4 = '' then begin
                            ...
                            s3 := SubStr(s3, 1, '.');
                          {$ifdef CodeJak}
                            if (not IsTextEqual(s3, AnsiString('iBmadExcept'))) or
                          {$else}
