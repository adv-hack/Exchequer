// ***************************************************************
//  madExcept.pas             version:  3.0l  ·  date: 2010-01-07
//  -------------------------------------------------------------
//  exception handling
//  -------------------------------------------------------------
...
// 2010-01-07 3.0l (1) fixed bug in SendShellMail
...
// 2009-09-15 3.0k Delphi/BCB 2010 support added
// 2009-07-21 3.0j (1) added %exceptClass%
...
const
  CMadExceptVersionString : PAnsiChar = '3.0l';
  CMadExceptVersionNumber =  3.11;
...
var
  OnExceptBoxCreate  : procedure (exceptBox    : dword;
...
// in dll modules secondary threads are not hooked by default
// in exe modules secondary threads are hooked by default
// these 2 functions allow you to overwrite the default behaviour
procedure HookThreads;
procedure DontHookThreads;
...
procedure ExpandVars(module: dword; var str: AnsiString; exceptClass, exceptMsg, bugReport: AnsiString);
...
begin
  ...
  if exceptMsg = '' then
    exceptMsg := 'Unknown';
  ReplaceText(str, '%exceptClass%', exceptClass);
  ...
  ReplaceText(str, '%computerName%', CompName);
...
begin
  {$ifdef CodeJak}
    result := FindResourceA(module, 'TCODEJAK', pointer(RT_RCDATA));
  {$else}
...
{$ifdef CodeJak}
  const CDfmHeader: PAnsiChar = 'TPF0' + #$8 + 'TCodeJak' + #$9 + 'iBmadExcept';
{$else}
        ...
        while pc1^ <> #0 do begin
          ...
          case (pc1 - 1)^ of
            ...
            #$14     : begin
                         ...
                         {$ifdef CodeJak}
                           MessageBoxA(0, PAnsiChar('Internal error: Invalid settings resource (' + IntToHexEx(dword((pc1 - 1)^)) + ').'), 'CodeJak', 0);
                         {$else}
...
begin
  InitSecAttr(sa, sd);
  {$ifdef CodeJak}
    result := CreateMutexA(@sa, false, PAnsiChar('CodeJakSettingsMtx' + IntToHexEx(GetCurrentProcessId)));
  {$else}
...
begin
  ...
  try
    if meSettingsBuf = nil then begin
      ...
      newMap := (map <> 0) and (GetLastError = 0);
      if map = 0 then
      ...
      if newMap then begin
        ...
        if map <> 0 then begin
          ...
          mesb := nil;
          {$ifdef CodeJak}
            MessageBoxA(0, 'internal error (opening settings buffer)', 'CodeJak', 0);
          {$else}
...
begin
  case action of
    nvaFormCreate : begin
                      {$ifdef CodeJak}
                        s1 := RegReadStr(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\ContactForm', 'RememberName');
                        s2 := RegReadStr(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\ContactForm', 'RememberEmail');
                      {$else}
    ...
    nvaFormClose  : if form.ModalResult = nvmOk then begin
                      ...
                      if check <> nil then
                        if check.Checked then begin
                          ...
                          if edit <> nil then
                            {$ifdef CodeJak}
                              RegWriteStr(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\ContactForm', 'RememberName', edit.Text);
                            {$else}
                          ...
                          if edit <> nil then
                            {$ifdef CodeJak}
                              RegWriteStr(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\ContactForm', 'RememberEmail', edit.Text);
                            {$else}
                              RegWriteStr(HKEY_CURRENT_USER, 'Software\madshi\ContactForm', 'RememberEmail', edit.Text);
                            {$endif}
                        end else begin
                          {$ifdef CodeJak}
                            RegDelVal(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\ContactForm', 'RememberName');
                            RegDelVal(HKEY_CURRENT_USER, 'Software\iBiSS\CodeJak\ContactForm', 'RememberEmail');
                          {$else}
...
begin
  ...
  ExpandVars(FModule, result, '%exceptClass%', '%exceptMsg%', '%bugReport%');
end;
...
begin
  ...
  if not IsProcessBlocked(250) then begin
//    len := sizeOf(buf);
//    if RegOpenKeyExA(HKEY_LOCAL_MACHINE, 'Software\Microsoft\Windows Messaging Subsystem', 0, KEY_READ, key) = 0 then begin
...
begin
  result := false;
  len := MAX_PATH;
  if RegOpenKeyExA(HKEY_CLASSES_ROOT, 'mailto\shell\open\command', 0, KEY_READ, key) = 0 then begin
...
function ExtractBugReportHeaderField(bugReport, field: AnsiString) : AnsiString;
var i1, i2 : integer;
begin
  result := '';
  i1 := PosStr(AnsiString(#$D#$A + field + ' '), bugReport);
  if i1 > 0 then begin
    i2 := PosStr(#$D#$A, bugReport, i1 + 10);
    if i2 > 0 then begin
      result := Copy(bugReport, i1, i2 - i1);
      i1 := PosStr(AnsiString(':'), result);
      Delete(result, 1, i1);
      TrimStr(result);
    end;
  end;
...
begin
  ...
  s1 := ExtractBugReportHeaderField(bugReport, 'exception class');
  s2 := ExtractBugReportHeaderField(bugReport, 'exception message');
  ExpandVars(settings.Module, subject, s1, s2, bugReport);
  ExpandVars(settings.Module, body,    s1, s2, bugReport);
    ...
    if settings.AttachBugReport then begin
      ...
      if s2 <> '' then begin
        ...
        if dosave then begin
          b1 := (settings.QueryInterface(IMEExceptionEx, exc) = 0) and
                (not exc.GetMailWasSaved);
          if not SaveBugReportEx(s2, bugReport, 0, true, settings) then
            DeleteFileA(PAnsiChar(s2));
          if b1 then
            exc.SetMailWasSaved(false);
        end;
...
begin
  ...
  if @OnExceptBoxDestroy <> nil then
    OnExceptBoxDestroy;
end;
...
begin
  case msg of
    ...
    WM_SYSCOMMAND        : if wParam and $FFF = SC_CLOSE then begin
                             if @OnExceptBoxDestroy <> nil then
                               OnExceptBoxDestroy;
                             result := 0;
                           end else
    ...
    {$ifdef noncommercial}
      ...
      WM_PAINT             : begin
                               ...
                               {$ifdef CodeJak}
                                 s1 := DecryptStr(#$75#$16#$3A#$31#$30#$1F#$34#$3E#$75#$25#$3A#$22#$30#$27#$30#$31#$75#$37#$2C#$75#$38#$34#$31#$10#$2D#$36#$30#$25#$21#$75#$E2#$75#$3B#$3A#$3B#$78#$36#$3A#$38#$38#$30#$27#$36#$3C#$34#$39#$75#$30#$31#$3C#$21#$3C#$3A#$3B#$75);
                               {$else}
...
procedure TExceptionBox.CheckCurrentTab;
  ...
  begin
    ...
    case length(columns) of
      3 : PrepareListView_Columns('^id|file name|priority|path');
      4 : PrepareListView_Columns('^id|file name|session|priority|path');
      5 : PrepareListView_Columns('^id|file name|gdi objects|user objects|priority|path');
      6 : PrepareListView_Columns('^id|file name|session|gdi objects|user objects|priority|path');
    end;
...
begin
  ...
  if defaultMsgBox then begin
    if FExc.ExceptType = etFrozen then begin
      ...
      ExpandVars(FExc.Module, errorTxt, FExc.ExceptClass, s1, '');
    end else
      errorTxt := s1;
...
begin
  ...
  ExpandVars(settings.Module, s1, ExtractBugReportHeaderField(bugReport, 'exception class'), ExtractBugReportHeaderField(bugReport, 'exception message'), bugReport);
...
begin
  ...
  if DetectConsole and IsConsole then begin
    if CGIApp_TCGIApplication_CGIHandleException = nil then begin
      ...
      ExpandVars(exceptIntf.Module, s1, exceptIntf.ExceptClass, exceptIntf.ExceptMessage, '');
      WriteLn(s1);
  ...
  if FCreateBugReport and (FBugReportHeader.ItemCount = 0) then
    with FBugReportHeader do begin
      ...
      try
                      ...
                      {$else}
                        {$ifdef ver210}
                          {$ifdef bcb}
                            try
                              if GetImageNtHeaders(HInstance).OptionalHeader.MajorLinkerVersion <> 2 then
                                AddItem('compiled with', 'BCB 2010')
                              else
                                AddItem('compiled with', 'Delphi 2010');
                            except
                              AddItem('compiled with', 'Delphi 2010');
                            end;
                          {$else}
        ...
        {$endif}
        {$ifdef CodeJak}
          AddItem('CodeJak version', ciBJakVersionString);
        {$else}
...
var count : integer;
  ...
  begin
    result := '';
    try
      if RegOpenKeyExA(HKEY_LOCAL_MACHINE, 'Software\ODBC\ODBCINST.INI', 0, KEY_READ, key1) = 0 then begin
...
var ThreadsHooked : boolean = false;
...
begin
  if not ThreadsHooked then
    InitCreateThreadHook(MESettings(HInstance));
end;
...
type TEnumWindowRec = record windowName, callstackItem: AnsiString end;
  ...
  begin
    ...
    if not result then
      {$ifdef CodeJak}
        SendMessageTimeoutA(window, RegisterWindowMessage('iBCodeJakCommand'),
...
begin
  ...
  if window <> 0 then begin
    {$ifdef CodeJak}
      ewr.windowName := 'iBMsgHandlerWindow' + IntToHexEx(GetWindowThreadProcessId(window, nil));
      ewr.callstackItem := 'JUMPCALLSTACK;' + callstackItem;
    {$else}
  ...
  if mySettings.IsValid then begin
    ...
    if GetLastError = 0 then begin
      ...
      if map <> 0 then begin
        ...
        {$ifdef CodeJak}
          MessageBoxA(0, 'internal error (opening global buffer)', 'CodeJak', 0);
        {$else}
    ...
    if madExceptBuf = nil then begin
      ...
      {$ifdef CodeJak}
        MessageBoxA(0, 'internal error. please notify bug@madshi.net', 'CodeJak', 0);
      {$else}
              ...
              {$else}
                ...
                {$else}
                  ...
                  {$else}
                    ...
                    {$else}
                      {$ifdef ver210}
                        vcl := FindBpl(rtl, 'vcl', 140, '@Forms@TApplication@HandleException$qqrp14System@TObject');
                        if rtl = 0 then
                          rtl := FindBpl(vcl, 'rtl', 140, '@System@FreeMem$qpv');
                        clx := FindBpl(0, 'visualclx', 140, '@Qforms@TApplication@HandleException$qqrp14System@TObject');
                      {$endif}
                    {$endif}
                  {$endif}
                {$endif}
