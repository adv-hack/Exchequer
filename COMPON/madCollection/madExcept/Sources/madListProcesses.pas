// ***************************************************************
//  madListProcesses.pas      version:  1.0b  ·  date: 2009-07-21
//  -------------------------------------------------------------
...
// 2009-07-21 1.0b added gdi/user object count
...
var
  ...
  GetGuiResources          : function (process, flags: dword) : dword stdcall = nil;
...
var count  : integer;
  ...
  begin
    ...
    items[count].gdiobjects  := 0;
    items[count].userobjects := 0;
    ...
    if pid <> 0 then begin
      ...
      if ph <> 0 then begin
        ...
        if @GetGuiResources <> nil then begin
          items[count].gdiobjects  := GetGuiResources(ph, 0);
          items[count].userobjects := GetGuiResources(ph, 1);
        end;
        CloseHandle(ph);
      end;
    end;
  ...
  begin
    ...
    {$endif}
    if @GetGuiResources = nil then
      GetGuiResources := GetProcAddress(GetModuleHandle('user32.dll'), 'GetGuiResources');
    c1 := 0;
...
var pidLen, sidLen, priorityLen, fileNameLen, gdiLen, userLen : integer;
    i1 : integer;
    s1 : string;
begin
  result := '';
  ...
  for i1 := 0 to count - 1 do
    with items[i1] do begin
      ...
      s1 := UnicodeString(IntToStrEx(gdiobjects));
      if Length(s1) > gdiLen then
        gdiLen := Length(s1);
      s1 := UnicodeString(IntToStrEx(userobjects));
      if Length(s1) > userLen then
        userLen := Length(S1);
  ...
  for i1 := 0 to count - 1 do
    with items[i1] do begin
      ...
      if (gdiLen > 0) or (userLen > 0) then
        result := result + string(IntToStrEx(gdiobjects,  - gdiLen - 1, ' ')) +
                           string(IntToStrEx(userobjects, -userLen - 1, ' '));
