// ***************************************************************
//  madExceptWizard.pas       version:  3.0g  ·  date: 2009-09-15
//  -------------------------------------------------------------
//  IDE integration
//  -------------------------------------------------------------
...
// 2009-09-15 3.0g Delphi/BCB 2010 support added
...
begin
  if FCurrentPrj > -1 then
    with IOTAProject(Projects[FCurrentPrj].FPrj).ProjectOptions do begin
      ...
      {$ifndef ver120}{$ifndef ver130}{$ifndef ver140}{$ifndef ver150}{$ifndef ver160}{$ifndef ver170}
        // BDS 2006 (and up?) needs more than resetting 'MapFile'
        ModifiedState := true;
  ...
  try
    ...
    try
      ...
      if (not Reg_ValExists(HKEY_CURRENT_USER,  'Software\madshi\madExcept', 'dontTouchUses')) and
        ...
        if addUnit and (PosText('dontComplain', s1) = 0) then
...
begin
  ...
  if s1 <> '' then begin
    ...
    {$ifdef ver210}fh := FindFirstFileA(PAnsiChar(s1 + '\*140.bpl'), wfd);{$endif}
