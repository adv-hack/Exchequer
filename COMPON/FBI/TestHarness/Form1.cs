...
//            string Amend;
            string irMark = _broker.AddIRMark(ref docXml, SubmissionType);
      ...
      private void cmdPoll_Click(object sender, EventArgs e)
      {
         ...
         callback.ResponseReceived += new EventHandler(HandleCallbackResponse);
          // Get the correlation ID of the posted document
         XmlNodeList CorrelationIDList = _return.GetElementsByTagName("CorrelationID");
         CorrelationID = CorrelationIDList[0].InnerText;
         _guid = _broker.BeginPolling(callback, CorrelationID, "IR-AB-AB123", true, @"https://secure.dev.gateway.gov.uk/submission");
//         _guid = _broker.BeginPolling(callback, "ABCDEFGHIJKLMNOPQRSTUVWXYZ123456", "IR-AB-AB123", true, @"https://secure.dev.gateway.gov.uk/submission");
      ...
      private void HandleCallbackResponse(object sender, EventArgs e)
      {
		...
		{
//            _broker.Delete(CorrelationID, "IR-AB-AB123", true, @"https://secure.dev.gateway.gov.uk/polling");
           this.txtXML.Text =  _broker.Delete(CorrelationID, "IR-AB-AB123", true, @"https://secure.dev.gateway.gov.uk/poll");
        }
