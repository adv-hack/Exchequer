...
uses
  APIUtil, CSAPI, SpellRes, {SpellResDe,} Registry, ISpell;
...
begin
 ...
 if SpType = sptMSOffice then begin
   ...
   //PR: 22/03/2016 v2016 R2 ABSEXCH-17390 Free not needed as exception during constructor destroys the object
{     try
       Speller.Free;
     except end;}
     Exit;
...
begin
  ...
  if L>0 then
    ...
    begin
      S:= FBackMemo.Text;
      if StartPos+1 <= Length(s) //NF:
      then StrLCopy(Buf, @S[StartPos+1], EndPos-StartPos);
    end;
end;
      ...
      if FLangSupport then
        if LangPos<=StartPos then
          begin
            ...
            L:= GetMemoLanguage;
            Lang := L; //PR: 22/03/2016 v2016 R2 ABSEXCH-17390 Initialise Lang to avoid warning
            LangPos:= StartPos;
            repeat
...
{    if FShowFinishMessage and (FFinishMessage<>'') and (not FSpeller.FNotActive) then
    begin
      if not fcancelled
      then MsgBox(FFinishMessage + #13#13,mtInformation,[mbOK],mbOK,'Spell Checker');
    end;{if}
//  end;{if}
...
begin
  ...
  begin
    ...
    begin
      if ShowModal = mrCancel then Checker.FCancelled := TRUE;  //NF:
//      if not Checker.FCancelled then Showmessage('x');
//      else Showmessage('x');
    end else
    begin
      Show;
...
var
  ...
  Cmpnt: TComponent;
begin
  //PR: 22/03/2016 v2016 R2 ABSEXCH-17390 Initialise Cmpnt & CanCheck to avoid warnings
  Cmpnt := nil;
  CanCheck := False;
  StartButton.Hide;
  try
...
//       Registry.Free;
   ...
   if (ISpellPath = '') or not FileExists(ISpellPath + 'ISpell.exe') then
   begin
     MsgBox('The spell checking dictionary cannot be found.'#13#13
     ,mtError,[mbOK],mbOK,'Spell Check Error');
   end;
...
//Initialize ISpeller
 ...
 if Activated then begin
   if ReadFile(so_r, buf, SizeOf(buf), j, nil) then
   begin
//     FillChar(lpOverlapped, SizeOf(lpOverlapped), #0);
//     LockSize := SizeOf(buf);

//     if not UnlockFileEx(so_r, 0, Lo(LockSize), Hi(LockSize), lpOverlapped)
//     then ShowMessage('UnlockFileEx Failed');
//     if not UnlockFile(so_r, Lo(0), Hi(0), Lo(LockSize), Hi(LockSize))
//     then ShowMessage('UnlockFileEx Failed');


//     if LockFileEx(so_r, LOCKFILE_FAIL_IMMEDIATELY, 0, Lo(LockSize), Hi(LockSize), lpOverlapped)
//     then SpellCommand('!')
//     if LockFile(so_r, Lo(0), Hi(0), Lo(LockSize), Hi(LockSize))
//     then SpellCommand('!')
...
begin
  ...
  // NF : Added so that errors are reported
  // Mainly so that if 2 writes happen simultaneously,
  // you will get a retry message on one machine rather than a failure with no message
  Repeat
    WriteResult := WriteFile(si_w, buf, StrLen(buf), j, nil);
    if WriteResult then
    begin
      // Write Suceeeded
    end else
    begin
      // Write Failed
      if MsgBox('A spell checking command failed.'#13#13
      + 'Click "Retry" to try the command again.'
      , mtInformation, [mbCancel, mbRetry], mbRetry, 'SpellCommand Failed') = mrCancel
      then WriteResult := TRUE;
    end;{if}
  until WriteResult;
  j:=0;
end;
...
procedure TSpellerDialog2.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//  if bXClose then CancelButtonClick(CancelButton);
end;
