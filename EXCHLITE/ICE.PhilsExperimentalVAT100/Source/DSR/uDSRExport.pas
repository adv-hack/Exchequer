...
Type
  TDSRExport = Class
  Private
    Class Function PrepareFilesAndSendCIS(pMessageInfo: TMessageInfo; pXmlList: TXMLList): Longword;
    // 20/06/2013. PKR. Added function to handle VAT100 XML files.
    Class Function PrepareFilesAndSendVAT100(pMessageInfo: TMessageInfo; pXmlList: TXMLList): Longword;
  Public
...
Var
  ...
  lXmlList: TXMLList;
  lDb: TADODSR;
  lLog: _Base;            // Message Logger
  lMsg: TMessageInfo;
Begin
{
1. User requests DSR to invoke a bulk export operation from dashboard, providing
   a specific time period and type of record.
2. DSR looks up export plugin configuration information from ICE database and
   creates an instance of the export plugin from the GUID retrieved from the ICE
   database.
}
  ...
  Begin
    ...
    Case lMsg.Mode Of
      ...
      // 21/06/2013. PKR. Added for VAT 100 XML upload.
      Ord(rmVAT100):
        begin
          lPack := lDb.GetExportPackage(lMsg.Company_Id, cVAT100EXPORT);
        end;
    Else
      lPack := lDb.GetExportPackage(lMsg.Pack_Id, lMsg.Company_Id);
    End;
    ...
    If lPack <> Nil Then
    Try
      {update the status to avoid sending this message again in the same minute}
      ...
      Begin
        {send in Dripfeed mode, normally, CIS or VAT100
         CIS and VAT100 require a slight different version of sending due to the
         use of the FBI components }
        Case lMsg.Mode Of
          ord(rmDripFeed):
            Result := PrepareFilesAndSend(lMsg, lXmlList, rmDripFeed);

          Ord(rmCIS):
            Result := PrepareFilesAndSendCIS(lMsg, lXmlList);

          Ord(rmVAT100):
            result := PrepareFilesAndSendVAT100(lMsg, lXmlList);
        Else
          Result := PrepareFilesAndSend(lMsg, lXmlList, rmNormal);
        End;

        { If in Dripfeed and the e-mail was sent off, update the send date because
          next time the dsr is going to export date from this date until the
          current date}
        If Result = S_OK Then
        Begin
          { Treating CIS and VAT 100 reports differently.  The reason being that
            later, the incoming process will pick up this file and try to process
            polling information from the GGW }
          If lMsg.Mode = Ord(rmCIS) Then
          Begin
            lDb.UpdateOutBox(lMsg.Guid, 0, '', '', '', 0, lXmlList.Count, '', '',
              cCISSENT, lMsg.Mode, dbDoUpdate);

            { Call the CIS polling system. The cisreceiverguid helps to identify
    ...
    Begin
      ...
      Begin
        ...
        Try
          ...
          {send cis msg using dsrmail object which includes a function to call
           the sender using the cis outgoing guid information}
          If Result = S_OK Then
          Begin
...
// 21/06/2013. PKR.  Added for VAT 100 XML upload.
{-----------------------------------------------------------------------------
  Procedure: PrepareFilesAndSendVAT100
  Author:    Phil Rogers
-----------------------------------------------------------------------------}
Class Function TDSRExport.PrepareFilesAndSendVAT100(pMessageInfo: TMessageInfo;
  pXmlList: TXMLList): Longword;
Var
  lXml       : WideString;
  lXmlIndex  : Integer;
  ...
  lCompGuid  : string;
  lFileName  : String;
  lXmlDoc    : TXMLDoc; // Not to be confused with the TXMLDocument class
  lXmlHeader : TXMLHeader;
  lSystem    : TSystemConf;
  lLog       : _Base;
  lDb        : TADODSR;
Begin
  _CallDebugLog('Start sending VAT100 ...');

  Result := S_OK;
  lSystem := TSystemConf.Create;
  lLog := _Base.Create;

  // Attempt to connect to the dashboard database
  Try
    lDb := TADODSR.Create(_DSRGetDBServer);
  Except
    On e: exception Do
      lLog.DoLogMessage('TDSRExport.PrepareFilesAndSendVAT100', 0,
  ...
  If Assigned(lDb) And lDb.Connected Then
  Begin
    // Database is connected, so get the connection string
    lLog.ConnectionString := lDb.ConnectionString;
    ...
    Begin
      ...
      If Count > 0 Then
      Begin
        // Create an outbox directory consisting of the System outbox followed by the GUID from pmessageInfo.
        lOutputDir := IncludeTrailingPathDelimiter(lSystem.OutboxDir + GUIDToString(Guid));

        // If a directory of that name exists already, then dump it and its contents
        // As it's GUID based, it shouldn't exist, but better to check.
        If _DirExists(lOutputDir) Then
        Try
        ...
        Try
          ...
          // Process the list of XML files. Should only be one for VAT 100, so
          //  we can probably remove this loop.
          For lXmlIndex := 0 To Count - 1 Do
          begin
            Try
              lXmlDoc.Clear;
              lXml := '';
              Try
                // get the XML file name, or the xml itself from the current TXMLItem in the list
                lXml := Items[lXmlIndex].XML;  // from pXMLList
              Except
                lXml := '';
              End;
              ...
              Begin
                ...
                FillChar(lXmlHeader, SizeOf(TXmlHeader), #0);
                // Populate the XML header.
                _GetXMLHeader(lXmlDoc, lXmlHeader);

                _FillXMLHeader(lXmlHeader, Guid, lXmlIndex + 1, Count, From, To_,
                  lDb.GetExportPluginLink(Items[lXmlIndex].Pack_Id), 0);

                { set xml packet header...}
                // PKR. NOT SURE WE NEED TO DO THIS FOR VAT 100.
                If _SetXmlHeader(lXmlDoc, lXmlHeader) Then
                Begin
                  If lXmlDoc.Doc.xml <> '' Then
                  Begin
                  ...
                  Begin
                    ...
                    Break;
                  End; {else begin}
                End // If XML header set ok
                Else
                Begin
                  // Failed to set XML header
                  Result := cSETTINGXMLHEADERERROR;
                  Break;
                End; {else begin}
              End {If lXmlDoc.Doc.xml <> '' Then}
              ...
              // Avoid hogging the CPU by taking a breather after every 5th file}
              If (lXmlIndex Mod 5) = 0 Then
                Sleep(1);
            Except
              On e: exception Do
              Begin
          ...
          { Send VAT100 message using dsrmail object which includes a function
            to call the sender using the VAT100 outgoing GUID information}
          If Result = S_OK Then
          Begin
            Result := TDSRMail.SendVAT100Msg(From, To_, Subject, lFileName);
        ...
        Except
          ...
          Begin
            Result := cERROR;
            lLog.DoLogMessage('TDSRSERVER.PrepareFilesAndSendVAT100', Result,
              'An error has occurred while processing VAT100 XML messages. Error: '
              + e.message, True, True);
          End;
        End {try..except}
      End
  ...
  If Result <> S_OK Then
    lLog.DoLogMessage('TDSRSERVER.PrepareFilesAndSendVAT100', Result, '', True, True);
  ...
  _CallDebugLog('Finished sending VAT 100');
End;
