...
Uses Variants, IniFiles, Math, Sysutils, Activex, strutils,
  ...
  // 21/06/2013. PKR. Added for VAT 100 XML.
  , uDSRVAT100
  ;
...
Begin
  ...
  // Get entries written by the CIS and VAT100 threads.
  // Note that CIS messages will only be processed if that option is installed.
  lOut := Null;
  If Not Terminated Then
  begin
    // ABSEXCH-14371. PKR. 21/06/2013.
    // Get the messages.  This now needs to be done for all systems due to the
    //  addition of the VAT100 option.
    lOut := fDb.GetOutboxMessages(0, 0, cPENDING, 0, 0, lRes, False);
    If (lRes = S_OK) And Not Terminated Then
      ProcessOutboxList(lOut);

    // If the CIS system is installed, then process those messages.
    ...
    Begin
      ...
      With TDSRCIS.Create Do
        Resume;
    End; // If fSystem.UseCISSubsystem

    // ABSEXCH-14371. PKR. 21/06/2013.
    // All systems have VAT100, so process the messages
    // Create a VAT 100 thread that will check for new VAT100 XML files
    With TDSRVAT100.Create Do
    begin
      // Create starts the thread in a suspended state, so resume it.
      Resume;
    end;
  end;
end;
...
{-----------------------------------------------------------------------------
  Procedure: ProcessOutboxList
  Author:    vmoura
  Description : Adds due messages to the schedule.
-----------------------------------------------------------------------------}
  ...
  Try
    ...
    Begin
      ...
      If (lTotal > 0) And Not Terminated Then
        ...
        Begin
          {create a message object based on the message in the olevariant array}
          ...
          Begin
            ...
            Begin
              ...
              Begin
                ...
                // Add the message to the schedule
                fSchedule.Add(lMsg);
              End
              Else
              Begin
...
{-----------------------------------------------------------------------------
  Procedure: Create
  Author:    vmoura
  Description : Processes messages in drip-feed mode.
-----------------------------------------------------------------------------}
...
Begin
  Result := False;
  Try
    // Check there are some messages to process.
    lTotal := _GetOlevariantArraySize(pMsgs);
    If (lTotal > 0) And Not Terminated Then
    Begin
      ...
      Begin
        ...
        // Create a message info structure
        ...
        Begin
          ...
          Begin
            ...
            Begin
              ...
              If (lMsg1.ScheduleId > 0) And
                 (lMsg1.Mode = Ord(rmDripFeed)) And
...
(*  If Assigned(fEmailLock) Then
  ...
  {get rid of emails older than one day and not processed}
    ...
    {get rid of mails older than one day}
      ...
      Except
        On e: exception Do
          _LogMSG('TDSRCheckMail.Execute :- Error processing old Inbox/Outbox e-mails. Error: '
            + e.Message);
      End;
        ...
        {check incoming mail system}
          If _FileSize(_GetApplicationPath + cDSRDOWNLOAD) > 0 Then
            _fileExec(_GetApplicationPath + cDSRDOWNLOAD + ' ' + cDSRDOWNLOADGUID,
              True, False)
          Else
            _LogMSG('TDSRCheckMail.Execute :- Download E-Mails application not found. Application name: '
              + cDSRDOWNLOAD);
        Except
          On e: Exception Do
            _LogMSG('TDSRCheckMail.Execute :- Error processing mail list. Error: '
              + e.message);
        End; {try}
      ...
      Begin
        If fDb.OutboxMessageStatusExists(0, cCISSENT) Then
          ...
          End
          Else
            _LogMSG('TDSRCheckMail.Execute :- CIS Receiver application not found. Application name: '
              + cCISRECEIVER);
      End; {If fDb.GetSystemValue(cISCISPARAM) = '1' Then}

      // ABSEXCH-14371. 28/06/2013.
      // Load the VAT 100 incoming application.  This applies to all users - no
      //  need to test whether AVT 100 is installed or not.
      If fDb.OutboxMessageStatusExists(0, cCISSENT) Then
      begin
        If _FileSize(_GetApplicationPath + cVAT100RECEIVER) > 0 Then
          Begin
            _fileExec(_GetApplicationPath + cVAT100RECEIVER + ' ' + cVAT100RECEIVERGUID,
              True, False)
          End
          Else
            _LogMSG('TDSRCheckMail.Execute :- VAT 100 Receiver application not found. Application name: '
              + cVAT100RECEIVER);
      end; // if OutboxMessageStatusExists

        //DoPause;
    Except
        (*fEmailLock.EndWrite;*)
      On e: exception Do
        _LogMSG('TDSRCheckMail.Execute :- General processing e-mail error. Error: '
          + e.Message);
    End;
