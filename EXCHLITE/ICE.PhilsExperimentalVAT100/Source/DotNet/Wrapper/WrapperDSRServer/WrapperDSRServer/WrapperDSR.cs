...
namespace WrapperDSRServer
  ...
  internal class WrapperDSR : MarshalByRefObject, IWrapperDSR
    ...
    public uint DSR_ViewVAT100Response(Guid pOutboxGuid, Guid pFileGuid, out string pXML)
      {
      DebugFileCtrl debug = new DebugFileCtrl(false);
      pXML = "";
      try
        {
        if (Program.LogEvents)
          debug.WriteLog("Calling DSR_ViewVAT100Response from within the remoting server. Guid: " + instanceID.ToString());
        return GlobalDSR.WithDSR().DSR_ViewVAT100Response(pOutboxGuid, pFileGuid, out pXML);
        }
      catch (System.Exception err)
        {
        debug.WriteLog("EXCEPTION (DSR_ViewCISResponse): " + err.Message);
