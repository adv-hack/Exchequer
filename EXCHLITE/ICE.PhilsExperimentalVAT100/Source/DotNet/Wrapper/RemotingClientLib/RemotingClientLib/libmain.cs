...
namespace RemotingClientLib
  ...
  public interface IDSRClient
    ...
    uint DSR_ViewVAT100Response(string MachineName, int PortNumber, Guid pOutboxGuid, Guid pFileGuid, out string pXML);
  ...
  public class DSRClient : IDSRClient
    ...
    //-------------------------------------------------------------------------
    // 24/06/2013. PKR. Added for VAT 100 XML support.
    public uint DSR_ViewVAT100Response(string MachineName, int PortNumber, Guid pOutboxGuid, Guid pFileGuid, out string pXML)
      {
      uint result = GENERIC_FAIL;
      pXML = "";
      try
        {
        IWrapperDSR _dsr = (IWrapperDSR)Activator.GetObject(typeof(IWrapperDSR), "tcp://" + MachineName + ":" + PortNumber.ToString() + "/WrapperDSR");
        result = _dsr.DSR_ViewVAT100Response(pOutboxGuid, pFileGuid, out pXML);
        }
      catch (Exception err)
        {
        throw err;
