{-----------------------------------------------------------------------------
 Unit Name: uADODSR
 Author:    vmoura, extended by Phil Rogers
 Purpose:   Provides database functions for the Dashboard
 History:
   Date      Who   Description
 21/06/2013  PKR   Added methods for handling VAT 100 XML files.
-----------------------------------------------------------------------------}
Unit uADODSR;
...
{$ENDIF}
  ...
  Public
    ...
    Function GetVAT100MessageDetail(pGuid: TGuid): Olevariant;
...
(*    Function SetWeekSchedule(pGuid: TGuid; pStartTime, pEndDate: TDateTime;
    ...
    // VAT 100 Functions
    Function UpdateVAT100(pGuid: TGuid; Const pIrMark, pCorrelationId, pClassType,
      pRedirection, pFileGuid: String; pPollingTime: Integer; pAction: TDBOption):
      Boolean;
    Function  HasVAT100Record(pGuid: TGuid): Boolean;
    Procedure AddVAT100DefaultEmailAccount;
    Procedure RemoveVAT100DefaultAccount;
  Published
...
//                    instance). Reinstated the original code.
  ...
  If pStartUp Then
    ...
    Begin
      _LogMSG('TADODSR.Create :- The Dashboard database does not exist.');
  ...
  Case pAction Of
    ...
    dbDoAdd:
      Begin
        ...
        // Build the SQL command to add a new record to the outbox.
        ...
        // Add the values to be inserted in the new record.
        lSql := lSql + ' (' + QuotedStr(GUIDToString(pGuid)) + ', ' +
                              inttostr(pCompany) + ', ' +
                              QuotedStr(Trim(pSubj)) + ', ' +
                              QuotedStr(Trim(pUserFrom)) + ', ' +
                              QuotedStr(Copy(Trim(pUserTo), 1, 255)) + ', ' +
                              inttostr(pPackId) + ', ' +
                              inttostr(pTotal) + ', ' +
                              inttostr(pStatus) + ', ' +
                              QuotedStr(lP1) + ', ' +
...
{-----------------------------------------------------------------------------
  ...
  Author:    vmoura
-----------------------------------------------------------------------------}
Function TADODSR.GetOutboxMessages(pCompany: Integer;
                                   pPackId: Longword;
                                   pStatus: Smallint;
                                   pDate: TDatetime;
                                   pMaxRecords: Longword;
                                   Out pResult: LongWord;
                                   Const pLogMessage: Boolean): OleVariant;
Var
  lSql: String;
Begin
  pResult := S_OK;
...
{-----------------------------------------------------------------------------
  Procedure: GetVAT100MessageDetail
  Author:    vmoura
-----------------------------------------------------------------------------}
Function TADODSR.GetVAT100MessageDetail(pGuid: TGuid): Olevariant;
Var
  lSql: String;
  lCount: Integer;
Begin
  Result := null;
  lSql := ' SELECT vat.outbox_guid, vat.irmark, vat.correlationid, vat.classtype, ' +
          '        vat.polling, vat.redirection, vat.fileguid FROM vat100 vat, ' +
          '        outbox ou ' +
          ' WHERE  vat.outbox_guid = ou.guid ' +
          ' AND    vat.outbox_guid = ' + QuotedStr(GUIDToString(pGuid));
...
Begin
  ...
  Begin
    lSql := 'insert into exppackage (company_id, description, fileguid, userreference, pluginlink) '
      + ' values (' + inttostr(pCompany) + ', '
                    + QuotedStr(Trim(pDescription)) + ', '
                    + QuotedStr(GUIDToString(pGuid)) + ', '
                    + IntToStr(pUserReference) + ', '
                    + QuotedStr(Trim(pPluginLink)) + ')';
    ...
    Try
      ...
      {0- exchequer code, 1- description, 2-company path}
        ...
        Begin
          ...
          If (lCompId = 0) Or ((lCompId <> 0) And (lCompPath <> lPAth)) Then
          Begin
...
// 21/06/2013. PKR. Added for VAT 100 XML handling.
//==============================================================================
// VAT 100 methods
//==============================================================================
{-----------------------------------------------------------------------------
  Procedure: UpdateVAT100
  Author:    Phil Rogers
-----------------------------------------------------------------------------}
Function TADODSR.UpdateVAT100(pGuid: TGuid; Const pIrMark, pCorrelationId,
                              pClassType, pRedirection, pFileGuid: String;
                              pPollingTime: Integer; pAction: TDBOption): Boolean;
Var
  lSql: String;
  lId: Integer;
Begin
  ...
  // Get outbox entry that applies to the GUID
  lId := SearchOutboxEntry(pGuid);
  ...
  Case pAction Of
    ...
    dbDoAdd:
      Begin
        lSql :=
          'INSERT INTO vat100 (outbox_id, outbox_guid, irmark, correlationid, ' +
          'classtype, polling, redirection, fileguid) VALUES ';
        lSql := lSql + ' (' + inttostr(lId) + ', ' +
                              QuotedStr(GUIDToString(pGuid)) + ', ' +
                              QuotedStr(pIrMark) + ', ' +
                              QuotedStr(pCorrelationId) + ', ' +
                              QuotedStr(pClassType) + ', ' +
                              inttostr(pPollingTime) + ', ' +
                              QuotedStr(pRedirection) + ', ' +
                              QuotedStr(pFileGuid) + ' ) ';
        Result := Exec(lSql, True);
      end;
    ...
    dbDoDelete:
      Begin
        lSql := 'DELETE FROM vat100 WHERE outbox_id = ' + inttostr(lId);
        lSql := lSql + ' OR outbox_guid = ' + QuotedStr(GUIDToString(pGuid));
        Result := Exec(lSql, False);
      end; {begin}
  end; {Case pAction Of}
...
{-----------------------------------------------------------------------------
  ...
  Author:    Phil Rogers
-----------------------------------------------------------------------------}
Procedure TADODSR.AddVAT100DefaultEmailAccount;
Var
  lAcc: TEmailAccount;
  lESystem: TEmailSystem;
Begin
  DeleteAllEmailAccounts;
  lAcc := TEmailAccount.Create;
  lESystem := GetEmailSystemByServerType('VAT100');
  If lESystem <> Nil Then
  Begin
    lAcc.EmailSystem_ID := lESystem.Id;
...
{-----------------------------------------------------------------------------
  ...
  Author:    Phil Rogers
-----------------------------------------------------------------------------}
Procedure TADODSR.RemoveVAT100DefaultAccount;
Var
  lSql: String;
Begin
  lSql := ' DELETE FROM emailaccounts WHERE servertype = ' + QuotedStr(cVAT100);
  Exec(lSql);
end;

{-----------------------------------------------------------------------------
  Procedure: HasVAT100Record
  Author:    Phil Rogers
-----------------------------------------------------------------------------}
Function TADODSR.HasVAT100Record(pGuid: TGuid): Boolean;
Var
  lSql: String;
Begin
  Result := False;
  If _IsValidGuid(pGuid) Then
  Begin
    lSql := ' SELECT id FROM vat100 WHERE outbox_guid = ' +
              QuotedStr(GUIDToString(pGuid));
    Result := RecordExists(lSql);
  end; {if _IsValidGuid(pGuid) then}
end;
