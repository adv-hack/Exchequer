...
//  cDSRINCOMING = 'E3D204B8-390E-4B99-9575-309E5FEE3598';
  ...
  c3RDPRT = '3rdPRT';
  cCIS = 'CIS';
  // 21/06/2013. PKR. Added for VAT 100 XML support.
  cVAT100 = 'VAT100';
...
//  cADDONDIR = 'ADDON';
  ...
  cADOCONNECTIONSTR1 =
    ...
    'Data Source=%s;Provider=SQLOLEDB.1;Password=%s;Persist Security Info=False;User ID=%s;Initial Catalog=IrisCommunicationEngine';

    // This version logs in ok, but doesn't use the login/password... 
//    'Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=IRISCommunicationEngine;Data Source=BMTDEVGOV\SQLEXPRESS';

    // PKR. TEMPORARY.
(*
    'Data Source=BMTDEVGOV\SQLEXPRESS;'+
    'Provider=SQLOLEDB.1;'+
    'Password=%s;'+  // Don't reverse the order of Password and User ID!
    'User ID=%s;'+
    'Integrated Security=SSPI;'+  // Ignore Username and Password
    'Persist Security Info=False;'+
    'Initial Catalog=IRISCommunicationEngine;';
*)
  ...
  //----------------------------------------------------------------------------
  // Descriptions used in the database for the package type.
  //cDRIPFEED = 'Dripfeed';
...
//  cAUTOMATICDRIPFEEDPARAM = 'AutoDripFeed';
  ...
  //----------------------------------------------------------------------------
  // 21/06/2013. PKR. Added for VAT XML files

  cVAT100_QUALIFIER_NODE        = 'Qualifier';
  cVAT100_QUALIFIER_ERROR       = 'error';
  cVAT100_QUALIFIER_ACK         = 'acknowledgement';
  cVAT100_QUALIFIER_RESPONSE    = 'response';
  cVAT100_QUALIFIER_MESSAGE     = 'message';
  cVAT100_CLASS_NODE            = 'Class';

  cVAT100_DEFAULT_POLLING_TIME  = 10000;  // 10 seconds
  cVAT100_POLL_INTERVAL_ATTR    = 'PollInterval';
  cVAT100_POLLING_URL           = 'ResponseEndPoint';

  // The directory in which VAT 100 XML files should be dropped. (one per company,
  //  off the company root)
  cVAT100XMLDIR             = 'VAT100XML';
  // Location of messages received back from the GGW.
  cVAT100RETDIR             = 'VAT100RET';

  // Filename used for processing the VAT 100 return.
  cVAT100RETFILEPROCESS     = 'XMLVAT100return.xml';

  // Used as the Subject: field in dashboard messages.
  cVAT100RETURNSUBJECT      = 'VAT 100 return submission';

  // Used as the To: field in dashboard messages.
  cVAT100RECIPIENT          = 'VAT 100 Gateway';

  // Used by TDSRProducer when populating the list
  cISVAT100PARAM            = 'IsVAT';

  cUSEVAT100TESTPARAM       = 'UseVAT100Test';
  cVAT100XMLFILE            = 'VAT100.xml';
  cVATXMLNODE               = 'VAT100';

  cVAT100RECEIVER           = 'VAT100Receiver.exe';
  cVAT100RECEIVERGUID       = '{C250333D-DC57-400F-9EAA-4162A813C7FB}';

  // XML <qualifier> element values from the HMRC response messages
  cVAT100QUALIFIER_ERROR     = 'error';
  cVAT100QUALIFIER_ACK       = 'acknowledgement';
  cVAT100QUALIFIER_RESPONSE  = 'response';
  cVAT100QUALIFIER_MESSAGE   = 'message';
  //----------------------------------------------------------------------------
    ...
    EndYear: Byte;
{
  ...
  {kind of record}
  // 20/06/2013. PKR. Added VAT100 mode.
  TRecordMode = (rmNormal, rmSync, rmBulk, rmDripFeed, rmCIS, rmVAT100);
  { message options}
