...
// 21/06/2013. PKR. Added for VAT 100 XML files.
function _GetVAT100File(const pDir: String): String;
...
Var
  ...
  lFile: String;
begin
  // Generated VAT XML files have a name in the format VATyymm.xml, where
  //  yy is the last 2 digits of the year and
  //  mm is the month number including a leading zero where applicable.
  Result := '';
  lFile := IncludeTrailingPathDelimiter(pDir) + 'vat*.xml';

  if FindFirst(lFile, faAnyFile, lSearch) = 0 then
    Result := IncludeTrailingPathDelimiter(pDir) + ExtractFileName(lSearch.Name);

  Sysutils.FindClose(lSearch);
end; {function _GetVAT100File(const pDir: String): String;}
...
Var
  ...
  Msg: TMsg;
Begin
  targettime := longint(GetTickCount) + msecs;
  While targettime > longint(GetTickCount) Do
