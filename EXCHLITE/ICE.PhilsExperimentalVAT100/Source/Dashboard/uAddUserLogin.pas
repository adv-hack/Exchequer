...
const
  cINFO1 = '%s Change User Login';
  cINFO2 = '%s Add New User Login';
  cDESCRIPTION = 'Type the user name, login and password of a user to give them permission to use the %s.';

Type
  //TfrmAddUserLogin = Class(TForm)
  TfrmAddUserLogin = Class(TFrmbase)
    advPanel: TAdvPanel;
...
Uses uDsr, uDashSettings, udashGlobal, uCommon;
...
Begin
  ...
  Begin
    ...
    lblInfo.Caption := Format(cINFO1, [_GetProductName(glProductNameIndex)]);
    btnAdd.Caption := '&OK';
  End
  Else
    //lblInfo.Caption := 'Dashboard Add New User Login';
    lblInfo.Caption := Format(cINFO2, [_GetProductName(glProductNameIndex)])
End;
...
{-----------------------------------------------------------------------------
  ...
  Author:    vmoura
-----------------------------------------------------------------------------}
procedure TfrmAddUserLogin.FormCreate(Sender: TObject);
begin
  inherited;
  //
  ...
  lblDescription.Caption := Format(cDESCRIPTION, [_GetProductName(glProductNameIndex)]);
end;
