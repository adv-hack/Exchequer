...
Type
  TfrmOutboxFrame = Class(TfrmMailBoxBaseFrame)
    ...
    CheckVAT100Response1: TMenuItem;
    ...
    Procedure actViewVAT100ResponseExecute(Sender: TObject);
...
{-----------------------------------------------------------------------------
  Procedure: actViewCISResponseExecute
  Author:    vmoura
-----------------------------------------------------------------------------}
...
{-----------------------------------------------------------------------------
  Procedure: actViewVAT100ResponseExecute
  Author:    vmoura
-----------------------------------------------------------------------------}
// This is identical to the CIS resonse handler, but has been separated out
// in case we need to change it.
Procedure TfrmOutboxFrame.actViewVAT100ResponseExecute(Sender: TObject);
Var
  lMsg: TMessageInfo;
Begin
  Inherited;
  ...
  If lMsg <> Nil Then
  Begin
    // Uses same form as CIS.
    Application.CreateForm(TfrmCISResponse, frmCISResponse);

    With frmCISResponse Do
    Begin
      Caption := 'VAT 100 Response';
      lblInfo.Caption := 'VAT 100 Response';
      LoadXmlFileList(lMsg.Guid);
      ShowModal;
      Free;
    End; {with frmXMLViewer do}
...
(*      N1.Visible := Not glISCIS;
      ...
      {show option to load VAT 100 xml records}
      actViewVAT100Response.Visible := fDb.HasVAT100Record(lMsg.Guid) and
        glDSROnline and MenuActive And lSelectedCount And lCompActive;
      actViewVAT100Response.Enabled := actViewVAT100Response.Visible;
