...
i've changed the buttons from a normal Delphi tbutton (not so good) to
the advglownbutton...so, everything looks similar at the dashboard
until tms changes this bloody tbutton for a new one...

remove procedure register
change TADVTaskDialog -> TDashTaskDialog
...
procedure TDashTaskDialog.SetContent(const Value: string);
begin
...
procedure TDashTaskDialog.SetExpandedText(const Value: string);
begin
...
procedure TDashTaskDialog.SetFooter(const Value: string);
begin
...
procedure TDashTaskDialog.SetInstruction(const Value: string);
begin
