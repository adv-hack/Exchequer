...
Copy and paste version of ttaskdialog from tms
i've changed the buttons from a normal tbuttom (not so good) to
the advglownbutton...so, everything looks similar at the dashboard
until tms changes this bloody tbutton for a new one...

{***************************************************************************}
*)

unit uDashTaskDialog;
...
uses
  ...
  ExtCtrls, Controls, ComCtrls, PictureContainer, ComObj, ShellAPI, CommCtrl, AdvGlowButton;
...
type
  ...
  TDashTaskDialog = class(TComponent)
  private
    FTitle: string;
    FContent: string;
    FFooter: string;
  ...
  protected
    ...
    function GetButton(ButtonID: Integer; var TaskButton: TTaskDialogButton): TAdvGlowButton;
    procedure EnableButton(ButtonID: integer; Enabled: boolean);
    procedure ClickButton(ButtonID: integer);
  ...
  public
    ...
    destructor Destroy; override;
    procedure BuildTaskDialog(TaskDialog: TDashTaskDialog);
    procedure SetPositions;
  end;

  function AdvMessageDlgPos(TaskDialog: TDashTaskDialog; X, Y: Integer): Integer;
...
procedure TDashTaskDialog.Clear;
begin
  CommonButtons := [];
  RadioButtons.Clear;
  CustomButtons.Clear;
...
procedure TDashTaskDialog.ClickButton(ButtonID: integer);
begin
...
constructor TDashTaskDialog.Create(AOwner: TComponent);
begin
...
destructor TDashTaskDialog.Destroy;
begin
  FRadioButtons.Free;
  FCustomButtons.Free;
  inherited;
...
procedure TDashTaskDialog.EnableButton(ButtonID: integer; Enabled: boolean);
begin
...
function TDashTaskDialog.Execute: integer;
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            StringToWideChar(FCustomButtons.Strings[i], TBWS[i], sizeof(TBWS[i]));
        ...
        begin
          ...
          begin
            StringToWideChar(FRadioButtons.Strings[i], TRWS[i], sizeof(TRWS[i]));
        ...
        if FTitle <> '' then
        begin
          StringToWideChar(FTitle, wTitle, sizeof(wTitle));
          TaskDialogConfig.pszWindowTitle := wTitle;
        end;

        if FInstruction <> '' then
        begin
          StringToWideChar(FInstruction, wDesc, sizeof(wDesc));
          TaskDialogConfig.pszMainInstruction := wDesc;
        end;

        if FContent <> '' then
        begin
          StringToWideChar(FContent, wContent, sizeof(wContent));
          TaskDialogConfig.pszContent := wContent;
        end;

        if FFooter <> '' then
        begin
          StringToWideChar(FFooter, wFooter, sizeof(wFooter));
...
function TDashTaskDialog.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
function TDashTaskDialog.GetVersionNr: Integer;
begin
...
procedure TDashTaskDialog.SetCustomButtons(const Value: TStringList);
begin
...
procedure TDashTaskDialog.SetRadioButtons(const Value: TStringList);
begin
...
procedure TDashTaskDialog.SetVersion(const Value: string);
begin
...
var
  ...
  FooterIconIDs: array[TTaskDialogFooterIcon] of PChar = (IDI_ASTERISK, IDI_EXCLAMATION, IDI_QUESTION, IDI_HAND);
...
function CreateAdvMessageDlg(TaskDialog: TDashTaskDialog): TForm;
begin
  Result := nil;
  if not Assigned(TaskDialog) then
    Exit;

  Result := TAdvMessageForm.CreateNew(Application);
  with Result do
  begin
...
procedure TAdvMessageForm.BuildTaskDialog(TaskDialog: TDashTaskDialog);
var
  DialogUnits: TPoint;
...
begin
    ...
    begin
      ...
      begin
        ...
        Windows.DrawText( Canvas.Handle,
          ...
          DrawTextBiDiModeFlagsReadingOnly);
        with TextRect do ButtonWidths[B] := Right - Left + 16;
      end;
      if ButtonWidths[B] > ButtonWidth then
        ButtonWidth := ButtonWidths[B];

      i := FcmBtnList.Add(TAdvGlowButton.Create(Self));
      with TAdvGlowButton(FcmBtnList.Items[i]) do
      begin
        Name := ButtonNames[B];
        Parent := Self;
  ...
  begin
    ...
    begin
      ...
      j := FcsBtnList.Add(TAdvGlowButton.Create(Self));
      with TAdvGlowButton(FcsBtnList.Items[j]) do
      begin
  ...
  begin
    ...
    begin
      Name := 'Image';
      Parent := Self;
      if IconID = IDI_HAND then
      begin
        Picture.Bitmap.Handle := LoadBitmap(hInstance, 'TD_SHIELD');
      end
      else
...
begin
  ...
  begin
    ...
    begin
      CsBtnGroupWidth := CsBtnGroupWidth + TAdvGlowButton(FcsBtnList.Items[i]).Width{ + ButtonSpacing};
    end;
  ...
  begin
    CmBtnGroupWidth := CmBtnGroupWidth + TAdvGlowButton(FcmBtnList.Items[i]).Width{ + ButtonSpacing};
  end;
  ...
  begin
    for i:= 0 to FcsBtnList.Count-1 do
    begin
      with TAdvGlowButton(FcsBtnList.Items[i]) do
      begin
        Top := Y;
        Left := X;
        Inc(X, TAdvGlowButton(FcsBtnList.Items[i]).Width + ButtonSpacing);
        //if (i = 0) then
          //h := h + TButton(FcsBtnList.Items[i]).Height;
        if (TAdvGlowButton(FcsBtnList.Items[i]).Height > BtnH) then
          BtnH := TAdvGlowButton(FcsBtnList.Items[i]).Height;
      end;
    end;
    if (FcsBtnList.Count > 0) then
      FWhiteWindowHeight := TAdvGlowButton(FcsBtnList.items[0]).Top{ - (FVertSpacing div 2)};
  end;
  ...
  begin
    ...
    begin
      Top := Y;
      Left := X;
      Inc(X, TAdvGlowButton(FcmBtnList.Items[i]).Width + ButtonSpacing);
      //if (i = 0) then
        //h := h + TButton(FcmBtnList.Items[i]).Height;
      if (TAdvGlowButton(FcmBtnList.Items[i]).Height > BtnH) then
        BtnH := TAdvGlowButton(FcmBtnList.Items[i]).Height;
    end;

    if (FcmBtnList.Count > 0) then
      FWhiteWindowHeight := TAdvGlowButton(FcmBtnList.items[0]).Top{ - (FVertSpacing div 2)};
  end;
  ...
  begin
    ...
    if Assigned(FFooterIcon) then
      r := Rect(HorzMargin + 20, Y, Width - HorzMargin, Y + 26)
    else
      r := Rect(HorzMargin, Y, Width - HorzMargin, Y + 26);
...
constructor TAdvMessageForm.CreateNew(AOwner: TComponent);
var
...
begin
  ...
  for I := 0 to ComponentCount - 1 do
    if Components[I] is TAdvGlowButton then
      ButtonCaptions := ButtonCaptions + TAdvGlowButton(Components[I]).Caption +
        StringOfChar(' ', 3);
...
function TAdvMessageForm.GetButton(ButtonID: Integer; var TaskButton: TTaskDialogButton): TAdvGlowButton;
var
  i, j: Integer;
begin
  j := 0;
  ...
  begin
    ...
    if (j >= ButtonID) then
    begin
      TAdvGlowButton(FcmBtnList.Items[i]).Enabled := Enabled;
      Result := TAdvGlowButton(FcmBtnList.Items[i]);
      break;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          TAdvGlowButton(FcsBtnList.Items[i]).Enabled := Enabled;
          Result := TAdvGlowButton(FcsBtnList.Items[i]);
        end;
        break;
      end;
    end;
