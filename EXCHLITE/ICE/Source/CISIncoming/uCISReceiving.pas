...
//PL 29/06/2017 2017-R2 ABSEXCH-18566 Govlink - Change TEST URL
type
  TURLType =(urlLive, urlTest);
...
Uses ComServ, Sysutils,
  ...
  uConsts, uCommon, uDSRSettings, uAdoDSR, HMRCSubmissionSettings
...
function TCISReceiving.GetURL(Const URLType : TURLType): string;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if GOVLINK_LIVE_URL <> EmptyStr then
          Result := GOVLINK_LIVE_URL
        else
          _LogMSG('TCISReceiving.GetURL :-'+ ErrorMessageGovLive);
      end
      else
      begin
        if GOVLINK_DEV_URL <> EmptyStr then
          Result := GOVLINK_DEV_URL
        else
          _LogMSG('TCISReceiving.GetURL :-'+ ErrorMessageGovTest);
      end;
    end
    else
      _LogMSG('TCISReceiving.GetURL :- Error loading HMRCSubmissionSettings.XML' + ErrorReason );
  end;
end;
...
Begin
  If Assigned(fPosting) Then
    With pCis, fPosting Do
      ...
      // MH 12/03/2018 ABSEXCH-18566: Switched to separate Polling URL from Submission URL
      if UseTestGateway then
        Delete(CorrelationID, CISClassType, UseTestGateway, cGGWCONFIGTESTPOLL)
      else
        Delete(CorrelationID, CISClassType, UseTestGateway, cGGWCONFIGLIVEPOLL)
  Else
