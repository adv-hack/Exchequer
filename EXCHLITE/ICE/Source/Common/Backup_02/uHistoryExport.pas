...
type
  ...
  private
    ...
    function OpenHistoryAdjustments: LongInt;
...
begin
  ...
  { Make adjustments based on Outstanding Transactions }
//  Adjustments.UpdateHistory;
...
begin
  ...
  if (Res = 0) then
    { Open the History Adjustments table. }
    Res := OpenHistoryAdjustments;
...
function THistoryExport.OpenHistoryAdjustments: LongInt;
begin
  SetDrive := DataPath;
  { Open the History Adjustments table }
  ...
  if (Result = 0) then
  begin
    Result := Open_File(F[HistAdjF], DataPath + FileNames[HistAdjF], 0);
    if (Result <> 0) then
    begin
