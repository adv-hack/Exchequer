...
type
  ...
  { The TAlreadyExported class holds a list of Cost Centres, Departments, and
    Stock Groups that have already been exported, so that we don't needlessly
    duplicate them (as different transactions could refer to the same cost
    centres, etc). }
...
begin
  ...
  begin
    ...
    RecPfix := CostCCode;
    Subtype := CSubCode[IS_COST_CENTRE];
...
begin
  ...
  begin
    ...
    RecPfix := CostCCode;
    Subtype := CSubCode[IS_DEPARTMENT];
    Key := FullCCKey(RecPFix, SubType, Code);
    FuncRes := Find_Rec(B_GetEq, F[PwrdF], PwrdF, RecPtr[PwrdF]^, 0, Key);
    if (FuncRes = 0) then
    begin
...
begin
  ...
  Key := FullStockCode(Code);
  FuncRes := Find_Rec(B_GetEq, F[StockF], StockF, RecPtr[StockF]^, StkCodeK, Key);
  { Build a list of all the parent items -- see the Stock Items section in
    BuildRecord. }
...
begin
  ...
  //      XMLWriter.AddLeafTag('thjobcode',             oTrans.thJobCode);
      ...
      // CS-PRAC -- on transactions exported from Practice
      // CS-CLIENT -- on transactions exported from Client
      if EnterpriseLicence.elProductType = ptLITEAcct then
        XMLWriter.AddLeafTag('thoperator',        'CS-PRAC')
      else
      ...
      on E:Exception do
        DoLogMessage('TDripfeedExport.AddTransactionHeader: ' +
                     E.Message, cBUILDINGXMLERROR);
    end;
  finally
    oTrans := nil;
  end;
...
function TDripfeedExport.LoadFromDB: Boolean;
  ...
  function IsInDateRange(oTrans: ITransaction3): Boolean;
  var
    TransPeriod, DripFeedPeriod: Integer;
  begin
    TransPeriod    := (oTrans.thYear * 1000) + oTrans.thPeriod;
    DripFeedPeriod := (Tracker.DripFeed.EndYear * 1000) + Tracker.DripFeed.EndPeriod;
    Result := (TransPeriod <= DripFeedPeriod);
  end;
  ...
  begin
    ...
    if EnterpriseLicence.elProductType = ptLITEAcct then
      Result := (Pos(DocCode, ICEPracticeDocCodes) <> 0)
    else
      Result := (Pos(DocCode, ICEDocCodes) <> 0);
  end;

  function IsAuto(oTrans: ITransaction3): Boolean;
  begin
    Result := (oTrans.thRunNo = -1) or
              (oTrans.thRunNo = -2) or
              ((oTrans.thAutoSettings <> nil) and (oTrans.thAutoSettings.atAutoTransaction));
  end;

  function IncludeRecord(oTrans: ITransaction3): Boolean;
  begin
    Result := IsInDocCodeRange(oTrans.thOurRef) and
              not IsAuto(oTrans) and
              Tracker.IsNew(oTrans.thOurRef);
    if Result then
    begin
      { Do not export transactions which are themselves imports. Do this by
        checking the Last Operator field -- transactions exported from the
        Client will have this set to 'CS-CLIENT', transactions exported from
        the Practice will have this set to 'CS-PRAC'. }
      if (EnterpriseLicence.elProductType = ptLITEAcct) and
         (oTrans.thOperator = 'CS-CLIENT') then
      begin
        { We are running at the practice end, and this transaction is an
          import from the client. Do not export. }
        Result := False;
      end
      else if (oTrans.thOperator = 'CS-PRAC') then
      begin
        { We are running at the client end, and this transaction is an
          import from the practice. Do not export. }
        Result := False;
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    Key := oTrans.BuildYearPeriodIndex(Tracker.DripFeed.StartYear, Tracker.DripFeed.StartPeriod);
    FuncRes := oTrans.GetGreaterThanOrEqual(Key);
    while (FuncRes = 0) and IsInDateRange(oTrans) do
    begin
