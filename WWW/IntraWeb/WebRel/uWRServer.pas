...
const
  LogoutRedirect = 'http://www.exchequer.com';
  ContactString = 'Please contact your local Advanced Enterprise Software representative or exchequersupport@advancedcomputersupport.co.uk.';
...
type
  ...
  TDealerCode = (dcNil, dcPlugIn, dcPlugInRel, dcMCM, dcDaily, dcDirectors, dcSystem, dcSystemUC, dcToolkit, dcToolkitUC,
                        dcModules, dcModulesUC, dcResync, dcResetEnt, dcVectronRel, dcVectronDaily, dcSpecialFunction );
  ...
  private
    ...
    procedure PrepareCodeEmailEX(const pEspecialFuncPass: string);
  ...
  public
    ...
    function GetTodaySecurityLasts: String;
    ...
    Function GetSpecialFunctionPass(const pChallenge: String): String;

    function SendMessage(SendSet: TSendSet; const pSpecialFuncPassword: String): string;
...
uses IWInit, uWRIntEntx, uWRIntMods, uWRIntPlugs, uWRIntVect, uWRIntOther,
     ...
     , SFPasswordU  // special functions unit
     ;
...
function TUserSession.GetTodaySecurityLasts: String;
begin
  with TCodeGenerator.Create do
  begin
    Result := GetSecurityLast;
    free;
  end; {with TCodeGenerator.Create do}
end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            //PR: 16/07/2013 ABSEXCH-14438 Rebranding
            BuildMsg:= BuildMsg + ' (c) Advanced Enterprise Software';
...
function TUserSession.SendMessage(SendSet: TSendSet;
  const pSpecialFuncPassword: String): string;
var
  DisplayResult: String;
  AllowGet: Boolean;
  BuildMsg: String;
  MaxId: Integer;
  NewCustomSendVia: Integer;
begin
  {Emails and/or SMSes the required release code if the user has the necessary
   get permissions, otherwise emails an internal request message;}
  ...
  try
      if stEmail in SendSet then with msgMain do
      begin
        Subject:= 'Special Function Password for ' + CustName;
        PrepareCodeEmailEX(pSpecialFuncPassword);

        SMTPer.Connect;
        try
          SMTPer.Send(msgMain);
          EmailCodeLog(True, dcSpecialFunction, False, '');
          DisplayResult:= 'The Special Function Password was emailed successfully.' + #13#10#13#10
        except
          DisplayResult:= 'The Special Function Password was unable to be requested.' + #13#10#13#10;
        end;
        SMTPer.Disconnect;
      end;
      ...
      begin
        if SMSPhone <> '' then with WRData.qyPrimary do
        begin
          BuildMsg:= ' Special Functions Password: ' + pSpecialFuncPassword;
          ...
          SMSLog(dcSpecialFunction, False, '');
          DisplayResult:= DisplayResult + 'The Release Code was SMSed successfully.';
          REsult := pSpecialFuncPassword;
        end
        else DisplayResult:= DisplayResult + 'Your SMS phone number is unavailable. Please contact your distributor to update your SMS details.';
      end;
      ...
      begin
        NewCustomSendVia:= 0;
        if stEmail in SendSet then
          inc(NewCustomSendVia, 1);

        if stSMS in SendSet then
          inc(NewCustomSendVia, 2);
...
begin
  ...
  try
    ...
    Add('Exchequer version: ' + Embolden(Version));
    ...
    begin
      Add('');
      Add('Following resynchronisation of Exchequer systems, please return to WebRel and update your customer record with the new ESN number created.');
    end;
    Add('');
    Add('This is an unmonitored email address. Please do not reply to this address.');
...
procedure TUserSession.PrepareCodeEmailEX(const pEspecialFuncPass: string);
var
  EmbedStrings: TStringList;
  MsgIndex: Integer;
  AllowGet: Boolean;
begin
  ...
  try
    ...
    Add('');
    Add('The Special Function Password is: ' + Embolden(pEspecialFuncPass));
...
(*
    ...
    Body.Add('    <tr>');
    Body.Add('      <td width="10" bgcolor="eeeeee" valign="top"><img src="http://www.iris.co.uk/images/zip_transparent.gif" width="10" height="10"></td>');
    Body.Add('      <td width="760" bgcolor="eeeeee" valign="top"><img src="http://www.iris.co.uk/images/zip_transparent.gif" width="10" height="10"></td>');
    Body.Add('      <td width="10" bgcolor="eeeeee" valign="top"><img src="http://www.iris.co.uk/images/zip_transparent.gif" width="10" height="10"></td>');
    Body.Add('    </tr>');
    Body.Add('    <tr>');
    ...
    Body.Add('                <td><img src="http://www.iris.co.uk/images/html_news/TOPBAR1.GIF" width="760" height="42"></td>');
    Body.Add('              </tr>');
    Body.Add('            </table>');
    Body.Add('          </td>');
    Body.Add('        </tr>');
    ...
    Body.Add('        <tr>');
    Body.Add('          <td width="130" valign="top" height="269" background="http://www.iris.co.uk/images/structure/nav_background.gif">');
    Body.Add('            <img src="http://www.iris.co.uk/images/html_news/side.gif"><img src="http://www.iris.co.uk/images/sub_pages/grey_blend1.jpg">');
    Body.Add('          </td>');
...
//PR: 17/07/2013 ABSEXCH-14438 Rebranding
    Body.Add('<HTML><HEAD>');
    Body.Add('<TITLE>Advanced Business Solutions : WebRel</TITLE>');
    Body.Add('<style type="text/css">');
    Body.Add('body {');
    Body.Add('            margin-left: 0px;');
    Body.Add('            margin-top: 0px;');
    Body.Add('}');
    Body.Add('-->');
    Body.Add('</style>');
    Body.Add('<meta http-equiv="Content-Type" content="text/html; charset=utf-8">');
    Body.Add('</HEAD>');
    Body.Add('<BODY>');
    Body.Add('<TABLE cellSpacing=0 cellPadding=0 width=780 border=0>');
    Body.Add('  <TBODY>');
    Body.Add('    <TR vAlign=top>');
    Body.Add('      <TD height="20" colspan="5">');
    Body.Add('      <div align="center">');
    Body.Add('                          <img src="http://www.exchequer-secure.com/wrscripts/Files/images/webrel_header.jpg" width="780" height="100"></div>');
    Body.Add('    </TD>');
    Body.Add('    </TR>');
    Body.Add('    <TR vAlign=top>');
    Body.Add('      <TD width=9>&nbsp;</TD>');
    Body.Add('      <TD width=137>');
    Body.Add('');
    Body.Add('      <P>&nbsp;</P>');
    Body.Add('    </TD>');
    Body.Add('      <TD width=25>&nbsp;</TD>');
    Body.Add('      <!-- Start Main Body -->');
    Body.Add('');
    Body.Add('<TD>&nbsp;<TABLE cellSpacing=0 cellPadding=0 width=587 border=0>');
    Body.Add('            <TBODY>');
    Body.Add('              <TR vAlign=top>');
    Body.Add('');
    Body.Add('            <TBODY>');
    Body.Add('              <TR vAlign=top>');
    Body.Add('');
    Body.Add('          <TD width=404>');
    Body.Add('            <!-- InstanceBeginEditable name="Main Copy Area" -->');
    Body.Add('            <p align="left" class="style20 style44">&nbsp;</p>');
    Body.Add('            <p align="left" class="style20 style44">&nbsp;</p>');
    Body.Add('            <font face="Arial, Helvetica, sans-serif" size="2" color="#666666">');
    Body.Add('            <table width="500" border="0" cellspacing="0" cellpadding="2">');
    ...
    Body.Add('            </table>');
    Body.Add('            </font>');
    Body.Add('                  <!-- InstanceEndEditable --> </TD>');
    Body.Add('                <TD width=25>&nbsp;</TD>');
    Body.Add('');
    Body.Add('          <TD id=xAtBottom width=158>');
    Body.Add('            <p><BR>');
    Body.Add('                </p>                  </TD>');
    Body.Add('              </TR>');
    Body.Add('            </TBODY>');
    Body.Add('        </TABLE></TD>');
    Body.Add('      <TD width=22>&nbsp;</TD>');
    Body.Add('    </TR>');
    Body.Add('    <tr valign="top">');
    Body.Add('    <TD>&nbsp;</TD>');
    Body.Add('');
    ...
    Body.Add('      <TD>&nbsp;</TD>');
    Body.Add('      <TD>&nbsp;</TD>');
    Body.Add('                        <TD><font face="Arial, Helvetica, sans-serif" size="1" color="#666666">');
    Body.Add('                        Exchequer is a product of Advanced Enterprise Software Ltd,');
    Body.Add('trading as Advanced Business Solutions and an Advanced Computer Software Group plc ');
    Body.Add('company.<br>EXCHEQUER is a trade mark of Advanced Enterprise Software Limited. &#169; Advanced Enterprise Software ');
    Body.Add('Ltd 2013. All rights reserved.</font></td>');
    Body.Add('      <TD>&nbsp;</TD>');
    Body.Add('    </TR>');
    Body.Add('  </TBODY>');
    Body.Add('</TABLE>');
    Body.Add('</BODY><!-- InstanceEnd --></HTML>');
    Body.Add('<!--#include virtual="../Files/site/includes/inc_main_tidyup.asp" -->');
...
begin
  ...
  case CodeID of
    ...
    cidSpecialFunction: Result := 'Special Function Password';
  end;
end;
...
begin
  ...
  case DealerCode of
    ...
    dcSpecialFunction : REsult := 'Special Function Password'
  end;
end;
...
begin
  ...
  else if Module = tidEBanking then Result := eidEBanking
  else if Module = tidOutlookDashboard then Result := eidOutlookDashboard
  else if Module = tidImporter then Result := eidImporter
  //SSK 2018R1 06/02/2018 ABSEXCH-19687: changes for GDPR and Pervasive File Encryption
  else if Module = tidGDPR then Result := eidGDPR
  else if Module = tidPervFileEncryption then Result := eidPervFileEncryption;
...
function TUserSession.GetSpecialFunctionPass(
  const pChallenge: String): String;
begin
  REsult := '';
  with TSFPassword.Create do
  begin
    Challenge := pChallenge;
    Result := Response;
    Free;
  end; {with TSFPassword.Create do}
end;
