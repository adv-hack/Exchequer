...
uses IWAppForm, IWApplication, IWTypes, IWCompCheckbox, IWCompButton, IWExtCtrls,
  ...
  IWLayoutMgr, IWTemplateProcessorHTML, IWCompEdit;
...
uses uCodeIds, uPermissionIDs,
     ...
     uCodeGen, uWRInstrx, SysUtils, WebSec_TLB
...
procedure TfrmExtOther.bnSFGetCodeClick(Sender: TObject);
var
RelCode: string;
begin
  with UserSession, Security do
  begin
    {check how to send the code}
    if not isValidSendVia(GetSendSet) then Exit;
    if ExceededThreshold(cidSpecialFunction, '') then Exit;

    edSFSecCode.Text := Uppercase(edSFSecCode.Text);
    RelCode := GetSpecialFunctionPass(edSFSecCode.Text);

    {check the result password}
    if RelCode <> '' then
    begin
      AuditLog(RelCode, cidSpecialFunction, -1, 0, 0, '');

      if Validate(UserCode, pidReqSpecFunctionPass) = 0 then
        SendMessage(GetSendSet, RelCode);
    end; {if RelCode <> '' then}
  end; {with UserSession, Security do}
end;
