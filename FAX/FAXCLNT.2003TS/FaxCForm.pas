...
{This code was commented out. I have reinstated it 13/12/2001}
  ...
  if bCheck then begin
    ...
    if bDebug then begin
      for iPos := 0 to RPDev.Printers.count - 1 do begin
        {if IsWinNT then }ShowMessage('RPDev.Printers[' + IntToStr(iPos) + '] = ' + RPDev.Printers[iPos])

{        else begin
          RPItem := TRPDeviceItem(RPDev.Printers.Objects[iPos]);
          ShowMessage('RPDev.Printers.Objects[' + IntToStr(iPos) + '] = ' +
          RPItem.FDriverName);
          RPItem := nil;
        end;{if}
      end;
    ...
    if IsWinNT then
      begin
        ...
        if WinNTDLLCheck(PrintDriverDir) then begin {Check for Files (I Think)}
          ...
          for iPos := 0 to RPDev.Printers.count - 1 do begin
            if UpperCase(Copy(RPDev.Printers[iPos],1,12)) = 'PRINT TO FAX' then Result := TRUE;
          end;
        end;{if}
      end
    else begin
...
//      Result := RPDev.CheckForDriver(PrintDriverName);
  ...
  if bContinue then begin
    ...
    if PrintDriverDir = '' then begin
      ...
      PrintDriverDir := ExtractFilePath(ParamStr(0)) + 'DRVINST\';
    ...
    if IsWinNt then
      begin
        if bDebug then showmessage('InstallDriver32(' + PrintDriverDir + ')');
...
{          GetPrinterDriverDirectory(nil, nil, 1, DriverDir, 255, SizeNeeded);
      ...
      if bDebug then showmessage('InstallDriver(' + PrintDriverDir + PrintDriverFile + ')');
      InstallDriver(PrintDriverDir + PrintDriverFile);
    end;{if}
    ...
    case DrvInstallError of
      ecOK :
        MessageDlg('The Exchequer Fax Driver ''Print to Fax'' has been successfully installed.', mtInformation, [mbOK], 0);
...
var
  CloseApp : boolean;
  sFileName : string;
  iServicePackNo : integer;
  wvWindowsVersion : TWindowsVersion;
...
begin
  ...
  wvWindowsVersion := GetWindowsVersion(iServicePackNo);

{ if wvWindowsVersion in wvTSVersions then sExtraPipeName := WinGetUserName
  else sExtraPipeName := '';
sExtraPipeName := '';}
  ...
  if bDebug then ShowMessage('ServicePackNo : ' + IntToStr(iServicePackNo));

  if (wvWindowsVersion = wv2000) and (iServicePackNo >= 2) then begin
    {Windows 2000 Service Pack 2 or later}

    sFileName := WinGetWindowsDir + 'SYSTEM32\APFAXCNV.DLL';
    if FileExists(sFileName) then begin
      if FileAge(sFileName) < 722102689 then begin
        {Version of APFAXCNV.DLL too old}
        MsgBox('The Exchequer Faxing System requires updating on this machine.' + #13#13
        + 'Please contact your Exchequer Support for further instructions on how to upgrade it.'
        + #13#13 + 'The Fax System is not compatible with your version of Windows.',mtWarning
        ,[mbOK],mbOK,'APFAXCNV.DLL - Windows 2000 SP' + IntToStr(iServicePackNo));
      end;{if}
    end;{if}
  end;{if}
...
begin // TfrmFaxClient.ApdFaxDriverDocStart
  ...
  {new Windows 2000 only fix}
  if GetWindowsVersion in [wv2000, wv2000TerminalServer] then
//  if GetWindowsVersion in [wv2000, wv2000TerminalServer, wvXP, wvXPTerminalServer] then
    begin
